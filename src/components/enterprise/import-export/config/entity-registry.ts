...
const MEMBER_FIELDS: FieldDefinition[] = [
  ...
  { key: 'status', label: 'Membership status', type: 'enum', enumValues: ['active', 'invited', 'suspended', 'removed', 'inactive'], enumLabels: { active: 'Active', invited: 'Invited', suspended: 'Suspended', removed: 'Removed', inactive: 'Inactive (legacy → suspended)' }, required: false, importable: true, exportable: true, group: 'Basic data', templateExample: 'active', description: 'active / invited / suspended / removed. Legacy inactive imports are mapped to suspended.' },
  // Organizational data
