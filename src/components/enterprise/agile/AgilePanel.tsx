      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-base flex items-center gap-2">
            <ListTodo className="h-4 w-4 text-primary" /> Agile integráció (Jira / Azure DevOps)
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
          {integrations.length === 0 ? (
            <p className="text-xs text-muted-foreground">
              Még nincs aktív integráció. Hozz létre egyet alább, majd tesztelt kapcsolat után válts vissza ide.
            </p>
          ) : (
            <div className="flex flex-wrap items-center gap-2">
              <span className="text-xs text-muted-foreground">Aktív integráció:</span>
...
function EmptyState() {
  return (
    <Card><CardContent className="py-8 text-center text-xs text-muted-foreground">
      Válassz aktív integrációt vagy hozz létre egyet a „Kapcsolatok" fülön.
    </CardContent></Card>
  );
}
