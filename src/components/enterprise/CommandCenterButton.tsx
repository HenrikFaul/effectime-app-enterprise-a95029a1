...
export function CommandCenterButton({ workspaceId, onOpenTab, recoveryMode }: Props) {
  const t = useT();
  ...
  const totalCount = counts.approvals + counts.onboarding + counts.access + counts.incompleteOrg;
      ...
      <PopoverContent align="end" className="w-[400px] p-0">
        <div className="p-3 border-b">
          <div className="flex items-center gap-2">
            <Activity className={`h-4 w-4 ${recoveryMode ? 'text-destructive' : 'text-primary'}`} />
            <div className="font-semibold text-sm">{t('command.title')}</div>
            {recoveryMode ? (
          ...
          <div className="text-[11px] text-muted-foreground mt-0.5">{t('command.subtitle')}</div>
        </div>
        <div className="p-3">
          {loading ? (
