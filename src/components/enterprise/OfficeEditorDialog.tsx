...
interface Props {
  workspaceId: string;
  officeId: string | null;  // null → create new
  open: boolean;
...
export function OfficeEditorDialog({ workspaceId, officeId, open, onOpenChange, onSaved }: Props) {
  const { t } = useI18n();

  const isNew = officeId === null;
  ...
  const loadMinStaffing = async (id: string) => {
    const { data } = await (supabase as any)
  ...
  useEffect(() => {
    ...
    if (isNew) {
      ...
      // Fetch the full office record, then load relations
      (async () => {
  ...
  const handleSave = async () => {
    ...
    if (isNew) {
      ...
      const { error } = await supabase.from('enterprise_offices').update(payload as any).eq('id', officeId!);
  ...
  const addEquipment = async () => {
    if (!newEquip.name.trim() || isNew || !officeId) return;
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
        ...
        <DialogFooter>
          <Button variant="ghost" onClick={() => onOpenChange(false)}>{t('office_mgr.btn_cancel')}</Button>
