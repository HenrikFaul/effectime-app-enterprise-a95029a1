...
- **BIZONYÍTOTT v3.51.5 candidate:** a tízfájlos focused suite 77/77,
  ...
  acceptance nem zöld.
- **BIZONYÍTOTT v3.51.6 source + hosted candidate:** az atomi member-profile read/save
  ...
  coverage 896/896, mobile 183/345/2 és drift 0 PASS. Draft PR #177 final commit
  `9dea63e733589a3debd7d6d794c353cb24a0e548` hosted run `29868681921` mind a
  10 jobja PASS, az Android és locked iOS simulator compile-lal együtt;
  release evidence `8510309445`, diagnostics `8510306792`, unsigned Android
  artifact `8510265497`.
- **BIZONYÍTOTT v3.51.7 source + hosted candidate:** a `BusinessRoleManager` legfeljebb
  20 canonical szerepet, pontosan 100,00 százalékos összallokációt és pontosan
  egy prioritást enged; a tagonkénti mentés ugyanazt az optimista, atomi
  member-profile RPC-t használja. A tenant-wide role delete szerveroldali
  tranzakcióban ellenőrzi újra a tenant/RBAC/entitlement határt, lockol,
  determinisztikusan újraoszt és csak exact postcondition mellett ad minimális
  audit receiptet.
- **BIZONYÍTOTT v3.51.7 source + hosted candidate:** a pre-Auth identity-provisioning
  saga négy állapotot és hat service-role-only RPC-t használ. A provisioning
  intent Auth user létrehozása előtt tartós, a kompenzáció DB-first, az Edge
  writerek és a cleanup worker ugyanazt az idempotens szerződést követik. Minden
  claimed `pending_auth` retry újra előkészíti a DB-cleanupot; a közös per-user
  write gate tiltja a membership/profile feltámasztását, a finalizer pedig
  workspace/user gate-ek alatt újratöröl, ellenőriz és atomikusan fejezi be a
  sagát. Két-session PostgreSQL contract igazolja a writer/finalizer sorosítást.
  Weben, Androidon és iOS-en továbbra is egyetlen közös React/Supabase
  adatforrás működik; nincs mobile-only adatbázis vagy privileged mobil secret.
  Bizonyíték: focused Vitest 99/99, Edge 60/60, entrypoint 30/30,
  runner 15/15, PostgreSQL 18.4 member-profile/business-role/identity-saga és
  typecheck PASS; teljes coverage 76 fájl / 992 teszt (51,27% statements/lines,
  75,11% branches, 46,39% functions); ESLint ratchet 1 148 error / 98 warning,
  11 javulással. Draft PR #178 final source commitja
  `f628d0b7d0931be5f16ebd03e14a608648077ac8`; hosted run `29879703648`
  10/10 PASS, release evidence `8514318251`, diagnostics `8514316593`, unsigned
  Android artifact `8514289059`. Ez source/CI és nem store/production bizonyíték.
- **BIZONYÍTOTT v3.51.8 source candidate + hosted:** a created-identity kompenzáció
  külön POST-only Edge workerbe került, per-job fencing tokennel, singleton
  run lease-szel, bounded deadline-nal és pontos receipt contracttal. Az
  owner-only ötperces scheduler installer dormant: a migráció nem hoz létre cron
  jobot, és a futáskori URL/credential/trigger secret Vaultból oldódik fel.
  A változás teljesen a közös Supabase Auth/RLS/RPC/Edge adatplatformon van;
  Androidhoz és iOS-hez nem készül külön adatbázis, cleanup API vagy privileged
  secret. Lokális bizonyíték: Edge 86/86, entrypoint/config 31/31, 65/65 pinned
  remote import, AST log-safety 9/9, runner 16/16 és mind az öt pinned PG18
  contract, köztük a fencing/single-flight/event-extension/temp-upgrade
  konkurenciavédelem. Draft PR #179 implementation head `1e3d874…`, hosted run
  `29885969841` 10/10 PASS, release evidence `8516558700`, diagnostics
  `8516557751`, unsigned Android artifact `8516531963`; ez nem store vagy
  production bizonyíték.
- **NO-GO:** a recurring identity-cleanup scheduler nincs telepítve. Restored-
