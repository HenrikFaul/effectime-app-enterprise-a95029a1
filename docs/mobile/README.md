...
| Candidate provenance             | **BIZONYÍTOTT:** a platform/CI csomag és az exact review-zott Swift lock candidate commitokban rögzített; a lokális és hosted release gate 363/363 PASS, a hosted Android+iOS compile zöld. | A kapuk required-checkként fenntartva; signed/store artifactnál SHA és signing provenance. |
...
| Native CI                         | **BIZONYÍTOTT:** run `29650123642` mind a hat jobja zöld; Android debug és release-evidence artifact megőrzött, iOS Xcode 26.5 compile PASS. | Required check, retention és protected-branch enforcement; signed release külön kapu. |
