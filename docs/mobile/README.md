...
- **BIZONYÍTOTT lokális I-26 candidate:** az admin leave override weben,
...
- **BIZONYÍTOTT lokális I-26 candidate:** a v2 same-key retryhoz actor/workspace-
...
- **BIZONYÍTOTT fail-closed I-26 candidate:** actor/workspace scope-onként egy
...
- **BIZONYÍTOTT hosted futásban:** a frontend, Edge, payroll PostgreSQL és
  Android job zöld lett. Az első iOS job Xcode-dal
  létrehozta és artifactként feltöltötte a `Package.resolved` fájlt, majd
  szándékosan leállt. A következő hosted run a review-zott, commitolt lockkal,
  a 363/363 strict release gate után, `-onlyUsePackageVersionsFromResolvedFile`
  módban és nulla tracked/untracked iOS drifttel `BUILD SUCCEEDED` eredményt adott.
...
| Candidate provenance             | **BIZONYÍTOTT I-25:** a platform/CI csomag és az exact review-zott Swift lock commitokban rögzített; a strict release gate 365/365 PASS, a hosted Android+iOS compile zöld. **I-26 PENDING:** új artifact/hosted bizonyíték még nincs. | A kapuk required-checkként fenntartva; signed/store artifactnál SHA és signing provenance. |
| Backend schema/API kompatibilitás | **BIZONYÍTOTT kódban:** az I-26 v2 additív és a v1-et változatlanul tartja; lokális PG18 v1/v2 smoke zöld. **BIZONYTALAN külső állapotban:** restored staging, production v2, PostgREST schema-cache és valós mobil retry nincs igazolva. | DB-first restored-staging és production rollout; exact v1/v2 signature/ACL; same-key replay; régi binary v1 smoke; client-second release és client-first rollback. |
...
| Native CI                         | **BIZONYÍTOTT I-25:** run `29678100165` mind a hét jobja zöld; Android debug, schema-2 release evidence és iOS Xcode 26.5 compile PASS. **I-26 PENDING:** az új admin-override PG18 jobbal 8/8 szükséges. | Required check, retention és protected-branch enforcement; signed release külön kapu. |
...
- **BIZONYÍTOTT lokális candidate:** az I-26 közös v2 adapter/outbox, a pinned
  PG18 contract, a 183/345 pontos mobil contract, a driftmentes Android/iOS sync
  és a 2/2 natív CSP smoke zöld. A helyi Android compile-ot az el nem fogadott
  SDK 35/36 licencek blokkolták a forrásfordítás előtt. **NO-GO deployra**, amíg
  a hosted Android/iOS gate, restored-staging DB-first rollout és kétplatformos
  fizikai transport-loss smoke nem igazolja ugyanazt a szerződést.
- **BIZONYÍTOTT:** a lokális implementáció, a commitolt candidate forrás és a
  review-zott Swift dependency lock fejlesztési foundationként **GO**, de a
