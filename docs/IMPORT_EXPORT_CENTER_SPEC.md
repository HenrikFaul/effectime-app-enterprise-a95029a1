...
- Row 2 (guidance row, styled gray): Data type hints and examples (e.g. `user@example.com`, `Teszt Felhasználó`, `Backend`, `Budapest Iroda`)
...
The user deletes row 2 (guidance) before adding data rows — or the importer recognizes and skips it automatically if it matches the guidance pattern.

**Recommendation**: Always include the guidance row in templates; auto-detect and skip it during import by checking if the second row's email column contains `@` — if not, treat row 2 as a non-data row and skip.
...
Max file size: 5 MB. Max rows: 2,000 per import. If larger, show guidance to split into batches.
...
- **Row 2 (guidance, styled gray)**: Human examples and type hints. Auto-skipped by importer.
