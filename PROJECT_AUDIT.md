...
Munkabranch: `codex/migration-provenance-recovery-wave1`
...
követett vagy új fájl, 149 jelenlegi enterprise UI-fájl, az I-31 candidate-del
133 helyi SQL-migráció és 31 Edge Function-könyvtár. A publikus oldal,
...
visszakerült, de a teljes Supabase clean replay még nem futott végig. A generált
...
| R-039 | Hiányzó remote migrációforrások bitpontos helyreállítása | linked history, generated types, későbbi feltétlen táblahivatkozások és remote statement-fetch | BIZONYÍTOTT source candidate | eredeti clock/marketplace SQL, kódba és manifestbe rögzített exact SHA-256/byte attesztáció, LF checkout contract, CI byte- és schema-provenance kapu | forrásban igen; linked DB-t nem ír | byte gate 9/9 + 2/2, schema parser 7/7, 133-migration exact gate PASS | igen | 61 shared / 72 local-only / 82 remote-only; production NO-GO | full Supabase replay, további remote-source recovery, forward-only ACL/tenant/secret hardening és restored staging |
...
| Recovered clock tenant-, credential- és adat-életciklus | A visszaállított `clock_events` append-only auditígéret mellett membership/workspace FK-kaszkáddal törölhető; a manuális `_office_id` nincs workspace-hez korrelálva; az `enterprise_offices.clock_in_nfc_tag` a jelenlegi aktív-member policy mellett böngészőből olvasható; a QR nem egyszer használatos és nincs szigorú TTL; GPS-koordinátákhoz nincs igazolt retention; clock state/idempotency contract sincs. A történeti SQL byte-fagyasztott, ezért csak új, forward-only migráció javíthatja. **BIZONYÍTOTT source finding.** | clock DDL/RPC/RLS, office credential, GDPR/audit | cross-tenant referencia, QR/NFC replay, payroll-audit adatvesztés, duplikált vagy sorrendhibás esemény és korlátlan helyadat-megőrzés; L | kompozit tenant-kényszerek; private hash/HMAC-alapú NFC credential; TTL+single-use QR; explicit retention; állapotgép+idempotency; két-tenant, concurrency, cascade és GDPR deletion/export contract; restored-staging rollback/runbook |
| Recovered marketplace secret-, entitlement- és tenant-határ | A `workspace_installed_plugins.config` minden aktív workspace-tag számára olvasható, miközben a mintamanifest webhook URL-t deklarál; az install RPC nem ellenőrzi szerveroldalon az Enterprise entitlementet és approved plugint is telepíthet; a `plugin_webhook_events.workspace_id` és `installed_plugin_id` független FK; nincs bizonyított explicit browser ACL-szűkítés, payload/response retention vagy URL/SSRF contract. A történeti SQL byte-fagyasztott. **BIZONYÍTOTT source finding.** | marketplace DDL/RPC/RLS, secrets, webhook delivery | secret-kitettség, entitlement-bypass, cross-tenant routing-integritás és PII/payload megőrzési kockázat; L | config secret Vault/private storage + safe view/RPC; szerveroldali entitlement és published-only install; kompozit tenant FK; explicit ACL/search_path; audit+retention+input bound+SSRF és concurrency teszt; forward-only migráció, restored-staging acceptance |
...
| Schema provenance gate                  | I-31 LOKÁLIS PASS KORLÁTTAL | Unit 7/7 és exact-debt gate PASS 133 migráción: 25/1/41/2 ismert unproven surface. Az I-27–I-30 candidate RPC-k, private worker state és a `profile_revision` nincsenek generated types-ban, ezért az exact local interface nem helyettesíti a hiteles linked-schema regenerálást. |
...
| Clean lokális migration replay          | NEM IGAZOLT             | A korábbi `plugin_webhook_events` source-hiány megszűnt. A friss DB-only Supabase replay a legelső migrációnál a hiányzó Storage-sémán állt meg; teljes Supabase-stack 133/133 replay még nem futott. |
...
local-only / 82 remote-only. Migráció-history repair vagy linked write nem
történt.
