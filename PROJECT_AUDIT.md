...
Munkabranch: `codex/plugin-config-secret-boundary`
...
követett vagy új fájl, 149 jelenlegi enterprise UI-fájl, az I-33 candidate-del
135 helyi SQL-migráció és 31 Edge Function-könyvtár. A publikus oldal,
...
A következő forward-only candidate a plugin-konfigurációkat privát táblába
mozgatja és a publikus kompatibilitási oszlopot `{}` értékre redaktálja; a
linked adatbázisra ez sem lett alkalmazva.
A generált
...
- Mind a 135 lokális migráció, generált Supabase típusok, RLS/policy-k, RPC-k,
...
| R-041 | Plugin-konfiguráció browser-secret határa közös web/Android/iOS adatúton | történeti public JSONB + active-member SELECT policy + webhook URL manifest + runtime caller inventory | BIZONYÍTOTT source candidate | forward-only private table/backfill; public `{}` CHECK; atomikus install/reinstall; cascade uninstall; enabled + workspace-páros service getter; browser AST caller gate | candidate-ben igen; linked DB-t nem ír | runner 11/11; pinned PG17.6: 15 korábbi + 7 új preflight és 3 reapply tamper, known-role ACL repair, legacy JSON-shape/tenant/role/atomicity/apply+reapply; browser caller 1/1; full 77/996 | igen | v3.51.11 local candidate; production NO-GO | restored staging és DB-first rollout; Vault/rotation/restore; entitlement/published-only, webhook tenant FK/SSRF/retention következő hullám |
...
| Recovered marketplace secret-, entitlement- és tenant-határ | A történeti `workspace_installed_plugins.config` minden aktív workspace-tag számára olvasható, miközben a mintamanifest webhook URL-t deklarál. Az I-33 candidate a raw JSON-t privát táblába másolja, a publikus mezőt `{}`-ra kényszeríti, eltávolítja a shared browser callerből, és workspace-páros/enabled service gettert ad; column ACL driftet javít, elveszett private sornál fail-closed. A linked DB-t nem írta. Az install RPC továbbra sem ellenőrzi a dokumentált Enterprise `plugin_install` entitlementet és `approved` plugint is telepíthet, bár a UI csak `published` elemet mutat; a webhook workspace/installation FK független, payload/response retention és SSRF contract nincs. A történeti SQL byte-fagyasztott. **BIZONYÍTOTT source finding + local contract.** | marketplace DDL/RPC/RLS, secrets, webhook delivery | linked rolloutig secret-kitettség; utána private plaintext/Vault, entitlement-bypass, cross-tenant routing-integritás és PII/payload megőrzési kockázat; L | I-33 restored-staging/DB-first apply; következő P1: azonos RPC-shape mellett server-side `plugin_install` és exact `published` kapu, status-lock, grandfathered install megőrzés és entitlement-vesztés utáni uninstall; majd Vault, kompozit tenant FK, audit+retention+input bound+DNS-aware SSRF és több-session contract |
...
| `npm run typecheck`                     | I-33 LOKÁLIS PASS       | A v3.51.11 current-tree typecheck minden web/mobile mounttal PASS. |
...
| `npm run mobile:check:source`           | I-33 LOKÁLIS PASS       | 183/183 source assertion: exact dependency integrity/kétplatformos plugin allowlist, release identity, CSP, identity/deep link/minimumok, auth/data-source/CI és natív build-artifact contract. |
| `npm run mobile:check`                  | I-33 LOKÁLIS PASS       | A v3.51.11 shared `dist-mobile` és Android/iOS artifact contract 345/345 PASS; ugyanaz a React/Supabase adatforrás. |
| `npm run test:e2e:mobile:built`         | I-33 LOKÁLIS PASS       | 2/2 landing/auth bridge-emuláció a v3.51.11 `dist-mobile` artifacton PASS. |
| `npm run mobile:sync` / release check   | I-33 PASS KORLÁTTAL     | Android/iOS sync és normalizálás PASS, natív drift 0; a 365 pontos release-check kizárólag a még nem commitolt candidate miatt állt meg, ezért a tiszta implementation commiton újrafuttatandó. |
...
| `npm test` / `npm run test:coverage`    | I-33 LOKÁLIS PASS       | Current tree: 77 fájl, 996/996 teszt; statements/lines 51,27% (45 444/88 636), branches 75,11% (2 176/2 897), functions 46,39% (322/694). |
| `npm run test:e2e:built`                | I-33 LOKÁLIS PASS       | A v3.51.11 built-web smoke 7/7 PASS. |
| `npm run build`                         | I-33 LOKÁLIS PASS       | A v3.51.11 production build PASS; nincs új runtime dependency. |
| `npm run bundle:check`                  | I-33 LOKÁLIS PASS       | Clean candidate build: JS 4 528 043 raw / ~1,297 MB gzip; largest 1 765 067 raw / ~561,4 KB gzip; CSS 180 862 / 29 600; az exact commitfüggő gzip gate a reviewed baseline alatt. |
| `npm audit --audit-level=moderate`      | I-33 LOKÁLIS PASS       | 0 ismert vulnerability. |
| `npm run security:secrets`              | I-33 LOKÁLIS PASS       | 1 539 tracked és nem ignorált untracked text file magas bizonyosságú current-tree scan + tracked mobil signing-key fájlnév tiltás; history scan még nyitott. |
| `npm run lint:ratchet`                  | I-33 LOKÁLIS PASS       | 1 148 error / 98 warning; a diagnosztikai fingerprint változatlan, új vagy elmozdított diagnosztika nincs. |
...
| Schema provenance gate                  | I-33 LOKÁLIS PASS KORLÁTTAL | Unit 7/7 és exact-debt gate PASS 135 migráción: 25/1/41/2 ismert unproven surface. Az I-27–I-33 candidate RPC-k és private táblák nincsenek hitelesen regenerált generated types-ban, ezért az exact local interface nem helyettesíti a linked-schema regenerálást. |
...
| Clean lokális migration replay          | NEM IGAZOLT             | A korábbi `plugin_webhook_events` source-hiány megszűnt. A friss DB-only Supabase replay a legelső migrációnál a hiányzó Storage-sémán állt meg; teljes Supabase-stack 135/135 replay még nem futott. |
