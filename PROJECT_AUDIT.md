...
Munkabranch: `codex/csv-import-error-boundary`
...
install/uninstall számlálórace-t friss statement-snapshotra épülő lock-protokollal
  ...
  Az I-43 v3.51.21 validated local candidate az import teljes request-határát valós
  ...
  PostgreSQL-contract lokálisan PASS; hosted CI és deploy még nincs állítva.
A generált
...
| R-051 | Bounded CSV-import failure, audit és spreadsheet-output boundary | raw fatal/tíz row-write provider message; hét prerequisite read hiba üres adatként kezelve, kettő raw provider részlettel leállva; ignored `import.started` insert; non-POST/invalid-JSON contract és formula-leading CSV reprodukció | BIZONYÍTOTT validated local source candidate | valós `Request` handler factory; zero-executor auth/RBAC/exact-feature denial; stabil additív code/request ID/no-store/CORS; bounded log/row error; mind a kilenc prerequisite read és start-audit fail-closed; CSV formula neutralization; AST structured-logger/console gate + külön provider-detail contract | igen a local candidate-ben; success shape változatlan, error code/header additív | focused 52/52; full 92/1116; Edge 109/109; entrypoint 31/31; type/lint/build/bundle; web 7/7; mobile 390/390 + 2/2; security/provenance/release és hét DB-contract PASS; hosted még nem futott | igen | v3.51.21 local candidate; commit/PR/hosted/deploy nincs; production NO-GO | hosted CI; transactional DB boundary, mutation-time authz/revoke és guaranteed failed/completion audit |
...
| CSV import atomitás, revoke-race és audit-tranzakció | Az I-43 candidate lezárja a raw fatal/row-write provider-detail, ignored prerequisite-read és ignored start-audit ágakat, de az exact entitlement-kapu továbbra is csak a kérés elején fut. A legfeljebb 2 000 soros service-role import soronként ír, ezért közbeni demotion/entitlement-revoke után folytatódhat, szerverhiba részleges eredményt hagyhat, a post-write completion-audit pedig a már megtörtént írást nem tudja visszagörgetni. **BIZONYÍTOTT inherited production blocker; a v3.51.21 bounded failure contract validated local candidate, nem atomitási javítás.** | `import-entity-data`, audit/error contract, web+mobile | jogosultságváltozás utáni írás, részleges commit és hiányos completion/failed audit; L | DB-oldali atomi importhatár és mutációkori authoritative recheck; konkurens demotion/revoke és rollback contract; guaranteed `import.failed`/completion audit; server dry-run/schema-version contract; staging acceptance |
...
| CSV import I-43 bounded failure contract | TELJES LOKÁLIS PASS | Focused 52/52; complete coverage 92 fájl / 1116 teszt; Edge 109/109 és `edge:check` 31/31; log-safety három funkción; ratchet 24/24; typecheck; lint 1,145/98 változatlan; build/bundle; web 7/7; mobile foundation 390/390 és E2E 2/2; hét DB-contract; security/provenance/release PASS. Edge identity `060c81a…` (95 fájl / 913,321 byte). Hosted CI még nincs igazolva. |
...
10. Az exact `csv_import` + `members_list` source-kapu az I-42 candidate-ben,
    ...
    I-43 validated local candidate-ben elkészült. Az I-43 hosted CI-je után a
    következő import-integritási csomag a specifikációban vállalt
    teljes tranzakciósság, a mutációkori authz/entitlement recheck, a UI által
...
A v3.51.21 I-43 jelenleg teljesen validált local source candidate. A valós `Request` handler
...
lokálisan PASS. Hosted CI még nem igazolt; commit, push, PR és deploy sem
történt. A szekvenciális írás,
