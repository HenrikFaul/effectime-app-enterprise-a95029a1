...
| I-12   | Quality Gate & Runtime Safety Foundation   | SHA-pinnelt CI, fingerprint lint és coverage/bundle ratchet, production-preview Playwright, PII logger, AuditLog exact field-allowlist, két SBOM/provenance, runbookok | nincs DB/API shape változás    | Edge 30/30, 0 diagnostic, raw check PASS, 64/64 exact import, 0 unpinned; Deno izolált az npm tree-től; egy commitban visszavonható | kész lokálisan, hosted CI függő |
...
| I-17   | Natív session storage és CSP hardening     | secure-store adapter/envelope/migráció/recovery, verified reset tombstone, logout fallback, raw-token link tiltás, explicit project/storage key, `dist-mobile`, CSP transform, mobile E2E és CI allowlist | 93/93 mobile target, 182/182 source és 342/342 artifact contract, 2/2 mobile E2E, 534/534 full suite, web 7/7, Android Gradle 276 task | web login/callback/storage és SEO/PWA kompatibilis; a közös logout hibakezelése szándékosan fail-closed módon erősödött; nincs DB/API shape; package/config/source normál reverttel visszaállítható, local purge explicit | kód, emulált regresszió, candidate provenance és Android compile/lint/assemble kész; release gate 1/351 NO-GO; iOS/hosted/fizikai evidence nyitott |
...
| `mobile:check:source` / `mobile:check`  | PASS                    | 182/182 source és 342/342 build/sync assertion: exact dependency integrity/kétplatformos plugin allowlist, teljes artifact-fájdfa és négy üres Capacitor shim SHA-256, CSP, identity/deep link/minimumok, auth/data-source/CI és iOS lock-bootstrap contract |
| `npm run mobile:check:release`          | VÁRT NO-GO              | Clean, commitolt candidate forráson 1/351 hiba marad: az iOS `Package.resolved` hiányzik; a kapu fail-closed működik és nem enged review-zatlan Swift graphot release-ként attestálni. |
...
| GitHub-hosted quality workflow          | NEM FUTOTT              | A SHA-pinnelt verify, `android-compile` és `ios-compile` jobok forrása, YAML-ja és 182 pontos source contractja PASS. Lock nélküli iOS PR review-artifactot generál és szándékosan fail; fordítás csak commitolt lockkal indul. Hosted bizonyíték csak commit/push/PR után lehetséges. |
