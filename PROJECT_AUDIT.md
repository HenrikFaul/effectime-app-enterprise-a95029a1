...
Munkabranch: `codex/recovered-surface-acl-hardening`
...
követett vagy új fájl, 149 jelenlegi enterprise UI-fájl, az I-32 candidate-del
134 helyi SQL-migráció és 31 Edge Function-könyvtár. A publikus oldal,
...
| R-040 | Recovered clock/marketplace explicit ACL, immutable search path és drift-attesztáció | linked catalog/ACL/policy/source inventory + recovered SQL + independent security review | BIZONYÍTOTT source + hosted candidate | forward-only PG17 migration; exact 10-policy, owner/config/source/pgcrypto preflight; browser least privilege; service-role contract preservation; QR exact extension binding | candidate-ben igen; linked DB-t nem ír | runner 11/11; pinned PG17.6, 13/13 tamper + repair/reapply/RPC/RLS smoke; invariants 34/34; full 76/995; web/mobile/release gates; hosted 11/11 PASS | igen | draft PR #181, implementation head `4046c8a…`, run `29891543713`; production NO-GO | history reconcile, restored staging, linked apply; plugin config safe projection/Vault; clock tenant/retention/idempotency wave |
...
| Recovered marketplace secret-, entitlement- és tenant-határ | A `workspace_installed_plugins.config` minden aktív workspace-tag számára olvasható, miközben a mintamanifest webhook URL-t deklarál; az install RPC nem ellenőrzi szerveroldalon az Enterprise entitlementet és approved plugint is telepíthet; a `plugin_webhook_events.workspace_id` és `installed_plugin_id` független FK. Az I-32 candidate az explicit browser ACL/search_path rést lokálisan lezárja, de linked apply nincs; payload/response retention és URL/SSRF contract továbbra sincs. A történeti SQL byte-fagyasztott. **BIZONYÍTOTT source finding + local contract.** | marketplace DDL/RPC/RLS, secrets, webhook delivery | secret-kitettség, entitlement-bypass, cross-tenant routing-integritás és PII/payload megőrzési kockázat; L | config secret Vault/private storage + safe view/RPC; szerveroldali entitlement és published-only install; kompozit tenant FK; I-32 restored-staging/linked ACL apply; audit+retention+input bound+SSRF és concurrency teszt; forward-only migráció, restored-staging acceptance |
...
A pinned PostgreSQL 17.6 contract 13/13 drift/tamper esetet állít meg, majd
...
Az aktív workspace-tag számára látható teljes plugin `config` külön P1 marad.

| Ellenőrzés                              | Eredmény                | Megjegyzés / fennmaradó kockázat                                                                                                                                   |
| --------------------------------------- | ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `npm ci --no-audit --no-fund`           | PASS                    | lockfile-ból determinisztikus install                                                                                                                              |
| `npm run typecheck`                     | I-32 LOKÁLIS PASS       | A v3.51.10 current-tree typecheck minden web/mobile mounttal PASS. |
...
| `npm run mobile:check:source`           | I-32 LOKÁLIS PASS       | 183/183 source assertion: exact dependency integrity/kétplatformos plugin allowlist, release identity, CSP, identity/deep link/minimumok, auth/data-source/CI és natív build-artifact contract. |
| `npm run mobile:check`                  | I-32 LOKÁLIS PASS       | A v3.51.10 shared `dist-mobile` és Android/iOS artifact contract 345/345 PASS; ugyanaz a React/Supabase adatforrás. |
| `npm run test:e2e:mobile:built`         | I-32 LOKÁLIS PASS       | 2/2 landing/auth bridge-emuláció a v3.51.10 `dist-mobile` artifacton PASS. |
| `npm run mobile:sync` / release check   | I-32 LOKÁLIS PASS       | Android/iOS sync és normalizálás PASS, natív drift 0; tiszta implementation commiton a release contract 365/365 PASS. |
...
| `npm test` / `npm run test:coverage`    | I-32 LOKÁLIS PASS       | Current tree: 76 fájl, 995/995 teszt; statements/lines 51,27% (45 444/88 636), branches 75,11% (2 176/2 897), functions 46,39% (322/694). |
| `npm run test:e2e:built`                | I-32 LOKÁLIS PASS       | A v3.51.10 built-web smoke 7/7 PASS. |
| `npm run build`                         | I-32 LOKÁLIS PASS       | A v3.51.10 production build PASS; nincs új runtime dependency. |
| `npm run bundle:check`                  | I-32 LOKÁLIS PASS       | JS 4 527 940 raw / 1 296 700 gzip; largest 1 765 067 / 561 428; CSS 180 862 / 29 600; reviewed baseline alatt. |
| `npm audit --audit-level=moderate`      | I-32 LOKÁLIS PASS       | 0 ismert vulnerability. |
| `npm run security:secrets`              | I-32 LOKÁLIS PASS       | 1 534 tracked és nem ignorált untracked text file magas bizonyosságú current-tree scan + tracked mobil signing-key fájlnév tiltás; history scan még nyitott. |
| `npm run lint:ratchet`                  | I-32 LOKÁLIS PASS       | 1 148 error / 98 warning; a diagnosztikai fingerprint változatlan, új vagy elmozdított diagnosztika nincs. |
...
| Schema provenance gate                  | I-32 LOKÁLIS PASS KORLÁTTAL | Unit 7/7 és exact-debt gate PASS 134 migráción: 25/1/41/2 ismert unproven surface. Az I-27–I-30 candidate RPC-k, private worker state és a `profile_revision` nincsenek generated types-ban, ezért az exact local interface nem helyettesíti a hiteles linked-schema regenerálást. |
...
| Clean lokális migration replay          | NEM IGAZOLT             | A korábbi `plugin_webhook_events` source-hiány megszűnt. A friss DB-only Supabase replay a legelső migrációnál a hiányzó Storage-sémán állt meg; teljes Supabase-stack 134/134 replay még nem futott. |
| Linked `db lint --level warning`        | FAIL                    | 7 error, 6 warning; read-only ellenőrzés                                                                                                                           |
| Live `effectime.app` release attestation | FAIL                    | A Lovable/Cloudflare root elérhető, de `/.well-known/effectime-release.json` továbbra is 404. A live artifact ezért nem köthető az I-27–I-31 SHA-k egyikéhez sem; production attestation NO-GO. |
...
61 shared / 72 local-only / 82 remote-only; a teljes 133-migrációs Supabase
