...
Munkabranch: `codex/export-pagination-integrity`
...
install/uninstall számlálórace-t friss statement-snapshotra épülő lock-protokollal
  ...
  (`29970235413`) 12/12 PASS, 0 annotáció, potential merge `edce77a…`; a
  dokumentációs final head kapuja még kötelező. A külön time-attendance payroll
  export továbbra is one-shot RPC-t castol és record előtt indít letöltést; ez a
  következő P1. Server-owned authorizáció/actor/snapshot/job és atomi audit
  nélkül production **NO-GO**.
A generált
...
| Legacy attendance payroll export teljessége és delivery-sorrendje | A time-attendance `fetchPayrollExport` egyetlen RPC-választ castol runtime séma/count/range nélkül; az AdminOverview a browser downloadot az `attendance_record_export` snapshot/státusz RPC előtt indítja. A központi XLS generátor-limit már védi a worksheetet, de a teljes `dataRows` tömb korábban elkészül, CSV hard cap nincs ezen az útvonalon, és a legacy gomb nem ellenőrzi külön a `payroll_export` entitlementet. **BIZONYÍTOTT külön P1 runtime gap; nem része az I-47 teljességi állításának.** | `time-attendance/api.ts`, `AdminOverview.tsx`, `types.ts`, payroll docs/RPC types | csonkolt vagy malformált béradat, audit/snapshot nélküli kiadott fájl, entitlement-paritás; L; RPC migration provenance és staging függőség | külön v3.51.26: exact pager + 23 mezős runtime schema + artifact cap; artifact→record→download orchestrator; entitlement/single-flight/stale-scope; 0/500/501/1001 és failure-order tesztek; majd atomi server prepare/export RPC staging contracttal |
...
`8549399088`. A külön attendance payroll one-shot cast + download-
before-record útvonala a következő P1; az authoritative server snapshot/audit,
linked history/staging és same-SHA live attestation hiánya miatt production
továbbra **NO-GO**.
