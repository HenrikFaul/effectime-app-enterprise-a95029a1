...
Munkabranch: `codex/plugin-install-count-concurrency`
...
követett vagy új fájl, 149 jelenlegi enterprise UI-fájl, az I-35 candidate-del
137 helyi SQL-migráció és 31 Edge Function-könyvtár. A publikus oldal,
...
install/uninstall számlálórace-t friss statement-snapshotra épülő lock-protokollal
  ...
  mellett globálisan reconciliálja; a lokális contract- és széles kapuk zöldek,
  de hosted CI és linked apply még nem futott.
A generált
...
| R-043 | Plugin install/uninstall számláló konzisztencia és meglévő drift helyreállítása | történeti uninstall statement-snapshot + determinisztikusan reprodukált `1 aktív / 0 tárolt` állapot + marketplace UI rendezés/megjelenítés | BIZONYÍTOTT source + local candidate | változatlan uninstall RPC-shape/OID/ACL; külön plugin `FOR NO KEY UPDATE`; következő statementben friss count; öt másodperces fail-closed lock-budget és drift-only globális reconciliation | candidate-ben igen; linked DB-t nem ír | runner 14/14; pinned PG17.6 végső teljes contract 95,6 s: 43 tamper, legacy red, globális repair, fixed green, grandfather/API/ACL/OID és dupla reapply; type/coverage/lint/build/web/mobile/Edge/security/provenance lokálisan PASS | igen | lokális v3.51.13 candidate; hosted CI függőben; production NO-GO | history reconcile; plugin-write drain; restored-staging DB-first apply + exact post-count; ugyanazon SHA deploy |
...
1 545-fájlos secret scan és 137-migration provenance lokálisan PASS. Hosted CI
még függőben; linked apply és production deploy nem történt.
...
| Release identity / Edge SBOM contract   | I-35 LOKÁLIS PASS; I-34 HOSTED PASS | I-35 release-identity 55/55 és Edge SBOM unit 7/7; deterministic web/mobile fingerprint, immutable Edge source-tree és pontos Edge inventory. A tiszta v3.51.13 manifest/SBOM és hosted artifact még a commit utáni kapu része. Az I-34 hosted release artifact `8522068999`. |
...
| `npm run mobile:check:source`           | I-35 LOKÁLIS PASS       | 183/183 source assertion: exact dependency integrity/kétplatformos plugin allowlist, release identity, CSP, identity/deep link/minimumok, auth/data-source/CI és natív build-artifact contract. |
| `npm run mobile:check`                  | I-35 LOKÁLIS PASS       | A v3.51.13 shared `dist-mobile` és Android/iOS artifact contract 345/345 PASS; ugyanaz a React/Supabase adatforrás. |
| `npm run test:e2e:mobile:built`         | I-35 LOKÁLIS PASS       | 2/2 landing/auth bridge-emuláció a v3.51.13 `dist-mobile` artifacton PASS. |
| `npm run mobile:sync` / release check   | I-35 LOKÁLIS PASS / CLEAN-HEAD FÜGGŐBEN | Android/iOS sync és normalizálás PASS, natív drift 0; a 365/365 fail-closed release assertion a tiszta implementation commit után fut. |
...
| `npm test` / `npm run test:coverage`    | I-35 LOKÁLIS PASS       | Current tree: 77 fájl, 996/996 teszt; statements/lines 51,27% (45 444/88 636), branches 75,11% (2 176/2 897), functions 46,39% (322/694). |
| `npm run test:e2e:built`                | I-35 LOKÁLIS PASS       | A v3.51.13 built-web smoke 7/7 PASS. |
| `npm run build`                         | I-35 LOKÁLIS PASS       | A v3.51.13 production build PASS; nincs új runtime dependency. |
| `npm run bundle:check`                  | I-35 LOKÁLIS PASS       | Candidate build: JS 4 527 932 raw / 1 296 729 gzip; largest 1 765 067 raw / 561 438 gzip; CSS 180 862 / 29 600; reviewed baseline alatt. |
| `npm audit --audit-level=moderate`      | I-35 LOKÁLIS PASS       | 0 ismert vulnerability. |
| `npm run security:secrets`              | I-35 LOKÁLIS PASS       | 1 545 tracked és nem ignorált untracked text file magas bizonyosságú current-tree scan + tracked mobil signing-key fájlnév tiltás; history scan még nyitott. |
| `npm run lint:ratchet`                  | I-35 LOKÁLIS PASS       | 1 148 error / 98 warning; a diagnosztikai fingerprint változatlan, új vagy elmozdított diagnosztika nincs. |
...
| `npm run edge:check` / ratchet / test    | I-35 LOKÁLIS PASS       | 31/31 entrypoint és config, 0 Deno diagnostic; 65/65 remote import exact, 0 unpinned; Deno 2.9.3; teljes Edge suite 86/86 PASS. Immutable source gate `5d38e629bc798af9e4536f51b819e2b99cd5378f95250478e09d05c3c26c6bce`, 89 fájl / 859 724 canonical byte. |
| Edge strukturált log és release SBOM    | I-35 LOKÁLIS PASS       | Adversarial AST log-safety 9/9, közvetlen és globális console/stdout/stderr escape tiltással; release identity 55/55 és Edge SBOM unit 7/7, a 31 deployálható entrypoint/config teljes leltárával. |
...
| Schema provenance gate                  | I-35 LOKÁLIS PASS | Parser unit 7/7; az exact-debt gate 137 migráción megtartotta a 25/1/41/2 ismert unproven surface baseline-t. Az I-27–I-35 candidate RPC-k és private táblák nincsenek hitelesen regenerált generated types-ban, ezért az exact local interface nem helyettesíti a linked-schema regenerálást. |
...
| Clean lokális migration replay          | NEM IGAZOLT             | A korábbi `plugin_webhook_events` source-hiány megszűnt. A friss DB-only Supabase replay a legelső migrációnál a hiányzó Storage-sémán állt meg; teljes Supabase-stack 137/137 replay még nem futott. |
| Linked `db lint --level warning`        | FAIL                    | 7 error, 6 warning; read-only ellenőrzés                                                                                                                           |
| Live `effectime.app` release attestation | FAIL                    | A Lovable/Cloudflare root elérhető, de `/.well-known/effectime-release.json` továbbra is 404. A live artifact ezért nem köthető az I-27–I-35 candidate SHA-k egyikéhez sem; production attestation NO-GO. |
...
Az I-34/I-35 következő kontrollált hulláma csak history reconcile után a restored-
staging DB-first acceptance. Az I-35 a determinisztikus concurrency harnesst és a
történeti install/uninstall count-race-t forrásszinten lezárja, de az általános
install lock-timeout/retry UX, a runtime feature-status/dependency,
downgrade-cleanup UI és grandfathered service-getter policy külön P2-k; ezek
hiánya nem lazíthatja vissza a szerveroldali entitlement/published-only határt.
...
61 shared / 76 local-only / 82 remote-only; a teljes 137-migrációs Supabase
...
A v3.51.13 I-35 lokális contract- és széles source/CI kapui zöldek, de ugyanezt a
NO-GO döntést tartja fenn; hosted CI, linked apply és production deploy még nem
történt.
