...
Munkabranch: `codex/csv-import-entitlement`
...
install/uninstall számlálórace-t friss statement-snapshotra épülő lock-protokollal
  ...
  Az I-42 v3.51.20 local candidate az aktív Import/Export Center Import módját és
  ...
  web/mobile, security és provenance lokálisan zöld. Hosted és production deploy
  még nincs állítva.
A generált
...
| R-050 | Exact `csv_import` + `members_list` entitlement az aktív import UI- és service-role íráshatáron | tier matrix + feature catalog + WorkspaceDashboard → ImportExportCenter → ImportWizard + Edge runtime call path | BIZONYÍTOTT local source candidate | közös fail-closed UI-döntés; entitlement-loss állapot/fókusz recovery; Edge tenant-feature preflight auth/RBAC után és audit/dry-run/read/write előtt; sanitizált 403/503 | igen a candidate-ben; Export változatlan | focused 23/23; Deno 10/10; Edge 96/96; full 90/1092; type/lint/build/bundle/web/mobile/security/provenance PASS | igen | local v3.51.20 candidate; production NO-GO | hosted gate; authenticated tier/outage acceptance; külön transactional/dry-run/schema-version csomag |
...
| CSV import atomitás, revoke-race és hibaredakció | Az I-42 exact entitlement-kapu csak a kérés elején fut; a legfeljebb 2 000 soros service-role import soronként ír, ezért közbeni demotion/entitlement-revoke után folytatódhat és szerverhiba részleges eredményt hagyhat. Több legacy hibaág nyers backend/DB `error.message` értéket ad vissza vagy logol. **BIZONYÍTOTT inherited production blocker; az I-42 új 403/503 ágai ettől függetlenül sanitizáltak.** | `import-entity-data`, audit/error contract, web+mobile | jogosultságváltozás utáni írás, részleges commit és információszivárgás; L | DB-oldali atomi importhatár és mutációkori authoritative recheck; bounded sanitizált hibakódok; konkurens demotion/revoke és rollback contract; `import.failed` audit; server dry-run/schema-version contract; staging acceptance |
...
| CSV import handler acceptance és template-export policy | Az I-42 helper/runtime contract 10/10 és a source-order teszt bizonyítja a gate elhelyezését, de nincs valós handler-szintű 403/503 + zero-audit/read/write request teszt. Az Import Wizard current-data template exportja `csv_import` mellett külön `export_center` nélkül elérhető; add-on/override esetén a kívánt privacy/termékpolitika nem bizonyított. | M | handler harness vagy staging request acceptance; explicit export-entitlement termékdöntés 2–3 opcióval; web/mobile adversarial teszt |
...
| CSV-import entitlement-paritás      | Az I-42 candidate az aktív Import módot és az `import-entity-data` hat szolgáltatásiszerepű mutációcsaládját exact `csv_import` + `members_list` union mögé zárja; lookup/malformed outage fail-closed, az Export változatlan. **BIZONYÍTOTT local source candidate:** focused 23/23, Deno 10/10, Edge 96/96, full 90/1092 és a teljes lokális release-kapu PASS. A specifikációban vállalt tranzakciósság, UI által használt szerver dry-run és `schema_version`-validáció ettől külön nyitott. | S/M | hosted gate; authenticated tier/outage acceptance; külön import-integritási csomag |
...
| I-42   | CSV import entitlement parity | exact fail-closed `csv_import` + `members_list` UI gate; accessible locked explanation; entitlement-loss dialog/state/focus recovery; Edge tenant-feature preflight auth/RBAC után és audit/dry-run/read/write előtt; sanitizált 403/503; Export változatlan | focused 23/23; Deno helper 10/10; Edge 96/96 és 31/31; full 90/1092; type/lint/build/bundle/web 7/7/mobile 390/390 + 2/2/security/provenance PASS | nincs DB/API/migration/dependency változás; teljes commit revert; az Edge gate önálló visszavonása csak endpoint-tiltással biztonságos | local v3.51.20 candidate; hosted pending; production NO-GO |
...
| CSV import entitlement I-42 gate | LOKÁLIS PASS | UI/source Vitest 23/23; Deno helper 10/10; teljes Edge 96/96; 31/31 entrypoint és 0/0 diagnostic; typecheck, 1 148/98 változatlan lint fingerprint és Edge identity `a5f6228…` / 91 fájl / 882 441 byte PASS. A széles kapu 90 fájl / 1092/1092, build, exact bundle 4 554 710 raw / 1 304 738 gzip JS és változatlan CSS, web 7/7, Android/iOS sync, mobile 390/390 + 2/2, 0 vulnerability, 1 575-fájlos / 0 találatos current secret scan és provenance kapuk PASS. Hosted és authenticated tier/outage acceptance nyitott. |
...
10. Az exact `csv_import` + `members_list` source-kapu az I-42 candidate-ben
    elkészült. A következő bizonyított import-integritási csomag a specifikációban
    vállalt teljes tranzakciósság, a UI által ténylegesen használt szerver
    dry-run és az explicit `schema_version` validáció. Ezt külön, API- és
    adatkontrakt-teszttel kell szállítani; az I-42 entitlement kapu nem lazítható.
...
A v3.51.20 I-42 local source candidate az exact `csv_import` + `members_list`
...
teljes 90 fájl / 1092/1092, type/lint/build/bundle, web 7/7, mobile 390/390 +
2/2, security/provenance és 0 dependency vulnerability PASS. Hosted, linked
apply és production deploy nem történt; a history/staging/Edge parity/same-SHA
blokkok miatt a kiadási döntés továbbra **NO-GO**.
