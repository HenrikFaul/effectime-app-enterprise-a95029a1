...
install/uninstall számlálórace-t friss statement-snapshotra épülő lock-protokollal
  ...
  az implementációs SHA 11/11 hosted Quality Gate-je zöld. Linked apply nem futott.
A generált
...
| Release identity / Edge SBOM contract   | I-35 LOKÁLIS + HOSTED PASS | Release-identity 55/55 és Edge SBOM unit 7/7; strict clean HEAD, deterministic web/mobile fingerprint, immutable Edge source-tree és pontos Edge inventory. A tiszta `cb0358e…` commitból lokális 62 928 byte-os manifest, 707 web- és 464 Edge-komponenses SBOM készült; hosted release artifact `8524406237`. |
...
| `npm run mobile:sync` / release check   | I-35 LOKÁLIS + HOSTED PASS | Android/iOS sync és normalizálás PASS, natív drift 0; a tiszta `cb0358e…` implementation commiton 365/365 fail-closed release assertion PASS. |
| Android unit/lint/assemble              | I-35 HOSTED PASS        | Run `29907202609` Android jobja PASS, artifact `8524356479`; ez unsigned candidate, nem store release. |
| iOS Xcode build                         | I-35 HOSTED PASS        | Run `29907202609` locked Swift package graphja és unsigned simulator buildje PASS; ez nem store release. |
