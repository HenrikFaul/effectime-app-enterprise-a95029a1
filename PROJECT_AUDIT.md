...
Munkabranch: `codex/ci-node24-actions`
...
install/uninstall számlálórace-t friss statement-snapshotra épülő lock-protokollal
  ...
  Az I-38 candidate a minden hosted jobon jelentkező Node 20 action-runtime
  deprecációt teljes SHA-ra rögzített Node 24 actionökkel és cache-policy
  regressziós contracttal váltja ki; a lokális forráskapuk zöldek, hosted
  elfogadás commit után kötelező.
A generált
...
| R-046 | GitHub Actions Node 24 runtime és változatlan cache/artifact szerződés | hosted run `29917203882` 11/11 zöld, de 11/11 Node 20 deprecation annotáció + hivatalos action release-ek | BIZONYÍTOTT local source candidate | `quality.yml`, `check-mobile-foundation.mjs`; checkout v5.1.0, setup-node v5.0.0, upload-artifact v6.0.0, setup-java v5.2.0 és setup-deno v2.0.5 immutable SHA-val | source-ban igen; app runtime és deploy útvonal változatlan | YAML PASS; source contract 216/216; Edge workflow 9/9; ESLint/diff/lint-ratchet PASS | igen | minden külső action exact Node 24 pinje és 6 no-cache / 5 explicit-cache contract kész; hosted pending | 11/11 hosted gate; 0 Node 20 annotáció; artifactnevek/retention és Node 22 igazolása |
...
- Az I-38 candidate lecseréli a 11 checkout, 11 setup-node és négy
  ...
  célzott ESLint, diff-check és a változatlan 1 148/98 lint-ratchet PASS.
...
| I-38   | GitHub Actions Node 24 runtime hardening | checkout v5.1.0, setup-node v5.0.0 és upload-artifact v6.0.0 signed release teljes SHA-n; meglévő setup-java v5.2.0/setup-deno v2.0.5 exact contract; Node 22 command runtime; hat DB-jobban automatic cache tiltás, öt jobban változatlan explicit npm cache | YAML PASS; mobile/CI source 216/216; Edge workflow 9/9; célzott ESLint, diff-check és lint-ratchet PASS; hosted pending | nincs DB/API/app dependency/bundle/runtime vagy artifact-shape változás; egy workflow/checker commit reverttel visszaállítható | local source candidate; hosted 11/11 és 0 Node 20 annotáció kötelező; production NO-GO |
...
| Release identity / Edge SBOM contract   | I-38 LOKÁLIS + I-37 HOSTED PASS | Current release-identity 55/55 és Edge SBOM unit 7/7; az I-38 exact-HEAD manifest commit után készül. Run `29917203882` I-37 merge-candidate manifestje 62 981 byte; web SBOM 707, Edge SBOM 464 komponens / 31 entrypoint. |
...
| `npm run mobile:check:source`           | I-38 LOKÁLIS PASS       | 216/216 source assertion: a korábbi 183 mobile contract mellett exact checkout/setup-node/upload/setup-java/setup-deno pin, az egyes action-családok referenciaszáma, a teljes 31 elemű action-allowlist és 6 no-cache / 5 explicit-cache workflow-szerződés. |
| `npm run mobile:check`                  | I-38 LOKÁLIS PASS       | A v3.51.16 shared `dist-mobile` és Android/iOS artifact contract 377/377 PASS; ugyanaz a React/Supabase adatforrás. |
| `npm run test:e2e:mobile:built`         | I-38 LOKÁLIS PASS       | 2/2 landing/auth bridge-emuláció a v3.51.16 `dist-mobile` artifacton PASS. |
| `npm run mobile:sync` / release check   | I-38 LOKÁLIS PASS / CLEAN-HEAD PENDING | Android/iOS sync és normalizálás determinisztikus, natív drift 0; exact-HEAD release contract és manifest commit után kötelező. |
| Android unit/lint/assemble              | I-37 HOSTED PASS        | Run `29917203882` Android jobja PASS, artifact `8528411950`; ez unsigned candidate, nem store release. |
| iOS Xcode build                         | I-37 HOSTED PASS        | Run `29917203882` locked Swift package graphja és unsigned simulator buildje PASS; ez nem store release. |
| `npm test` / `npm run test:coverage`    | I-38 LOKÁLIS PASS       | Current tree: 80 fájl, 1034/1034 teszt; statements/lines 51,60% (45 936/89 022), branches 75,48% (2 297/3 043), functions 47,71% (344/721). |
...
| `npm run test:e2e:built`                | I-38 LOKÁLIS PASS       | A v3.51.16 built-web smoke 7/7 PASS egy workerrel, köztük 320/390/768 px public clipping check; ez nem authenticated downgrade E2E. |
| `npm run build`                         | I-38 LOKÁLIS PASS       | A v3.51.16 production build 4092 modullal PASS; nincs új runtime dependency. |
| `npm run bundle:check`                  | I-38 LOKÁLIS + I-37 HOSTED PASS | I-38 local: JS 4 537 693 raw / 1 299 419 gzip; largest 1 767 981 / 562 199; CSS 180 900 / 29 610. Az explicit szűk baseline PASS; nincs nyitott plafon. |
| `npm audit`                             | I-38 LOKÁLIS PASS       | 0 ismert production vagy development vulnerability. |
| `npm run security:secrets`              | I-38 LOKÁLIS PASS       | 1 552 tracked és nem ignorált untracked text file magas bizonyosságú current-tree scan + tracked mobil signing-key fájlnév tiltás; history scan még nyitott. |
| `npm run lint:ratchet`                  | I-38 LOKÁLIS PASS       | 1 148 error / 98 warning; a diagnosztikai fingerprint változatlan, új vagy elmozdított diagnosztika nincs. |
| GitHub Actions Node 24 I-38 contract    | LOKÁLIS PASS / HOSTED PENDING | YAML parser PASS; minden action full SHA és exact contract; hivatalos action source `runs.using: node24`; checkout 11, setup-node 11, upload-artifact 4, setup-java 1, setup-deno 4; Node 22 marad; hat DB cache tiltott, öt job explicit npm-cache; artifact szerződés diffben változatlan. Hosted 11/11 és 0 Node 20 annotáció még kötelező. |
...
| `npm run edge:check` / ratchet / test    | I-38 LOKÁLIS PASS       | 31/31 entrypoint és config, 0 Deno diagnostic; 65/65 remote import exact, 0 unpinned; Deno 2.9.3; teljes Edge suite 86/86 és ratchet unit 23/23 PASS. Immutable source gate `27bdaca73e6d84b12c4d28aac81ae2797aaecec7ab1beade24ecb7c9aebe1180`, 89 fájl / 859 880 canonical byte. |
| Edge strukturált log és release SBOM    | I-38 LOKÁLIS PASS       | Edge log-safety PASS; release identity 55/55 és Edge SBOM unit 7/7, a 31 deployálható entrypoint/config teljes leltárával. |
...
| Migration byte provenance gate         | I-38 LOKÁLIS PASS       | Unit 9/9; két exact recovered SQL / 26 678 raw byte; a kanonikus byte+SHA a verifierben és a manifestben is rögzített; path, entry-set, type, symlink, length, SHA-256 és koordinált SQL+manifest drift fail-closed ellenőrzés; Git LF contract PASS. |
| Schema provenance gate                  | I-38 LOKÁLIS PASS | Exact-debt gate és 7/7 unit 138 migráción megtartotta a 25/1/41/2 ismert unproven surface baseline-t. Az I-27–I-36 candidate RPC-k és private táblák nincsenek hitelesen regenerált generated types-ban, ezért az exact local interface nem helyettesíti a linked-schema regenerálást. I-38 nem módosít sémát. |
...
| Clean lokális migration replay          | NEM IGAZOLT             | A korábbi `plugin_webhook_events` source-hiány megszűnt. A friss DB-only Supabase replay a legelső migrációnál a hiányzó Storage-sémán állt meg; teljes Supabase-stack 138/138 replay még nem futott. |
| Linked `db lint --level warning`        | FAIL                    | 7 error, 6 warning; read-only ellenőrzés                                                                                                                           |
| Live `effectime.app` release attestation | FAIL                    | A Lovable/Cloudflare root elérhető, de `/.well-known/effectime-release.json` továbbra is 404. A live artifact ezért nem köthető az I-27–I-36 candidate SHA-k egyikéhez sem; production attestation NO-GO. |
...
61 shared / 77 local-only / 82 remote-only; a teljes 138-migrációs Supabase
...
A v3.51.16 I-38 CI-only source candidate a deprecated Node 20 GitHub Action
...
hosted 11/11 és nulla Node 20 annotáció nélkül nem kész. Ez nem deploy workflow,
és a production **NO-GO** állapotát nem változtatja meg.
