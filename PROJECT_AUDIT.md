...
| ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ | ----- | ------------------------------------------------------------------- |
| ESLint-adósság                      | 1 209 error + 104 warning 178 fájlban; az I-22 9 errort, 4 warningot és 1 érintett fájlt szüntetett meg, az I-23 nem ad új findingot; a csökkentett fingerprint-baseline védi a javítást | XL    | modulonkénti csökkentés, baseline-frissítés, végül 0                |
...
| Bundle-méret                        | I-23 Windows candidate: JS 4 454 357 raw / 1 271 510 gzip; largest 1 740 007 / 551 967; CSS 180 798 / 29 589. Az I-22 Windows artifacthoz képest +1 444 raw / +328 gzip (+0,032% raw), largest +501/+125. A total plafon a mért Windows/Linux varianciával 4 454 485 / 1 271 789; largest-raw exact, largest-gzip/CSS nem nőtt. | M | további route split és regressziós mérés |
...
| I-23   | Leave-list fail-visible recovery és a11y | explicit lokalizált alert/retry, accessible loading/empty state, fail-closed adatürítés és a monoton query-generation megtartása | 8 új lista-state teszt + 7 meglévő race + 10 i18n contract; teljes 595/595 coverage, type/lint/build/bundle, web 7/7, mobile source 183/artifact 345/E2E 2, clean release contract 365 | nincs DB/API/dependency változás; komponens+i18n commit reverttel visszaállítható; raw backend hiba nem kerül DOM-ba | teljes lokális candidate és clean release contract PASS; hosted kapu nyitott |

## 7. Ellenőrzések és eredmények

Az I-22 commitolt főágán a hosted kapu 7/7 PASS. Az alábbi táblázat az erre épülő
I-23 lokális candidate aktuális állapotát is rögzíti; a tiszta I-23 release-artifact
és hosted kapu továbbra is külön bizonyíték. A dinamikus PG18 admin-override
acceptance nem része egyik frontend PASS-nak sem.
...
| `npm run mobile:check:release`          | PASS                    | Az I-23 clean, commitolt forrásán 365/365 fail-closed contract assertion PASS; a hosted-Xcode lock kizárólag Capacitor 8.3.1 és KeychainSwift 21.0.0 exact upstream URL/tagrevízióit engedi. |
...
| `npm test`                              | PASS (coverage runner)  | A teljes coverage-futtatás ugyanazt az 54/54 fájlban 595/595 tesztet futtatta zölden. |
| `npm run test:coverage`                 | PASS                    | 54/54 fájl, 595/595 teszt; statements/lines 47,71% (40 653/85 198), branches 65,75% (960/1 460), functions 32,40% (163/503). A nyolc új lista-state teszt a hiba/retry/a11y és stale-error kockázatot fedi. |
| `npm run test:e2e:built`                | PASS                    | I-23 izolált `/auth` 1/1 PASS (4,0 s), majd teljes 7/7 Chromium smoke PASS ugyanazon artifacton. Az első futás egy másik repository párhuzamos Playwright terhelése alatt egyszer 20 s navigációs timeoutot ért el assertion/runtime hiba nélkül; az izolált és teljes ismétlés ezt nem reprodukálta. |
| `npm run build`                         | PASS                    | Vite 6.4.3, 4 080 modul; nagy chunk warning; ugyanaz a forrás web és mobile artifactba épült |
| `npm run bundle:check`                  | PASS                    | I-23 Windows candidate: JS 4 454 357 raw / 1 271 510 gzip; largest 1 740 007 / 551 967; CSS 180 798 / 29 589. Az I-22 Windows artifacthoz képest +1 444 / +328 (+0,032% raw), largest +501/+125. A total ceiling a korábban mért 93 raw/151 gzip platformvarianciát, 35 byte raw kerekítési tartalékot és 128 byte gzip allowance-t tartalmaz; largest-gzip/CSS nem nőtt. |
...
| SBOM és release manifest                | PASS (lokális + hosted evidence) | Main `21b2fed…` schema-2 artifact `8437804119`: `dirty=false`, exact SHA, 72 fájl / 6 843 629 byte, web fingerprint `2837a4c…3184c`; shared web/mobile package-lock CycloneDX 707 + Edge/Deno 464 komponens, 125 migráció, 30 Edge entrypoint és exact native provenance. |
| GitHub-hosted quality workflow          | I-22 PASS / I-23 PENDING | Main run `29672580843`: mind a hét hosted job PASS a `21b2fed29913194cc6fcf4c7d26904ed48325959` merge SHA-n, beleértve a PostgreSQL 18.4, Android és iOS kaput. Az I-23 candidate hosted futása commit után kötelező. |
