...
Munkabranch: `codex/plugin-install-policy-boundary`
...
követett vagy új fájl, 149 jelenlegi enterprise UI-fájl, az I-34 candidate-del
136 helyi SQL-migráció és 31 Edge Function-könyvtár. A publikus oldal,
...
adatbázisra egyik sem lett alkalmazva.
A generált
...
- Mind a 136 lokális migráció, generált Supabase típusok, RLS/policy-k, RPC-k,
...
- Current-tree secret scan 1 542 követett és nem ignorált untracked szövegfájlon,
...
| R-042 | Plugintelepítés Enterprise-entitlementje és published-only szerződése | marketplace specifikáció `plugin_install` Enterprise mappingje + történeti RPC `approved` elfogadása + UI exact `published` szűrése | BIZONYÍTOTT, lokálisan validált source candidate | változatlan install RPC-shape; server-side `plugin_install` kapu; exact `published`; státusz sorlock alatti újraellenőrzése; meglévő installok és uninstall megőrzése | candidate-ben igen; linked DB-t nem ír | runner 12/12; pinned PG17.6: 40 fail-closed tamper, ebből 15 install-policy; státusz-race rollback, két-workspace konkurens count, reapply; full 77/996; type/build/web/mobile/security/Edge kapuk | igen | v3.51.12 lokálisan validált candidate; hosted CI és production deploy függő; production NO-GO | history reconcile; restored-staging DB-first apply; P2 stale install-count race, runtime feature-metaadat, downgrade UX, getter-policy és bounded lock-wait |
...
- Az I-34 candidate a meglévő plugin-install RPC alakjának megtartásával
  ...
  nem elhallgatott zöld. Hosted CI, linked apply és deploy még nem bizonyított.
...
| Plugin install/uninstall count race  | Az új install lock-sorrendje illeszkedik a történeti uninstall útvonalhoz, de a változatlan uninstall RPC READ COMMITTED statement-snapshotja egy konkurens install után elavult `install_count` értéket írhat. Ez meglévő hiba, nem v3.51.12 regresszió. | M | forward-only count-frissítés egyetlen konzisztens statementben vagy tranzakciós lock-protokollal; determinisztikus két-session install/uninstall contract; reconciliation query/runbook |
| Plugin concurrency-test determinism  | A célzott contract 4 másodperces `pg_sleep` barrierre, általános `wait_event_type = 'Lock'` jelre és nem keményen korlátozott async befejezésre támaszkodik. A viselkedés lokálisan bizonyított, de lassú hosztolt gépen flake-kockázat marad. | S/M | advisory barrier/receipt; exact lock target; explicit per-process timeout és cleanup; ismételt hosted stress |
| Bundle-méret                        | Current I-34 build: JS 4 527 932 raw / 1 296 729 gzip; largest 1 765 067 raw / 561 438 gzip; CSS 180 862 / 29 600. A reviewed bundle gate zöld, de az abszolút méret továbbra is jelentős. | M | további route split; hosted artifact összevetés; regressziós mérés |
...
Hosted CI, linked adatbázisírás és production deploy még nem történt; a kiadási
döntés history reconcile és restored-staging DB-first acceptance előtt
**NO-GO**.

| Ellenőrzés                              | Eredmény                | Megjegyzés / fennmaradó kockázat                                                                                                                                   |
| --------------------------------------- | ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `npm ci --no-audit --no-fund`           | PASS                    | lockfile-ból determinisztikus install                                                                                                                              |
| `npm run typecheck`                     | I-34 LOKÁLIS PASS       | A v3.51.12 current-tree typecheck minden web/mobile mounttal PASS. |
...
| Release identity / Edge SBOM contract   | I-34 LOKÁLIS PASS KORLÁTTAL | Release-identity 55/55 és Edge SBOM 7/7; strict full-SHA/clean HEAD, deterministic web/mobile fingerprint, immutable Edge source-tree és pontos Edge inventory. A v3.51.12 commit-specifikus manifest/SBOM artifact a tiszta commit kapuja. |
...
| `npm run mobile:check:source`           | I-34 LOKÁLIS PASS       | 183/183 source assertion: exact dependency integrity/kétplatformos plugin allowlist, release identity, CSP, identity/deep link/minimumok, auth/data-source/CI és natív build-artifact contract. |
| `npm run mobile:check`                  | I-34 LOKÁLIS PASS       | A v3.51.12 shared `dist-mobile` és Android/iOS artifact contract 345/345 PASS; ugyanaz a React/Supabase adatforrás. |
| `npm run test:e2e:mobile:built`         | I-34 LOKÁLIS PASS       | 2/2 landing/auth bridge-emuláció a v3.51.12 `dist-mobile` artifacton PASS. |
| `npm run mobile:sync` / release check   | I-34 PASS KORLÁTTAL     | Android/iOS sync és normalizálás PASS, natív drift 0; a 365 pontos release-check a tiszta implementation commiton újrafuttatandó. |
...
| `npm test` / `npm run test:coverage`    | I-34 LOKÁLIS PASS       | Current tree: 77 fájl, 996/996 teszt; statements/lines 51,27% (45 444/88 636), branches 75,11% (2 176/2 897), functions 46,39% (322/694). |
| `npm run test:e2e:built`                | I-34 LOKÁLIS PASS       | A v3.51.12 built-web smoke 7/7 PASS. |
| `npm run build`                         | I-34 LOKÁLIS PASS       | A v3.51.12 production build PASS; nincs új runtime dependency. |
| `npm run bundle:check`                  | I-34 LOKÁLIS PASS       | Candidate build: JS 4 527 932 raw / 1 296 729 gzip; largest 1 765 067 raw / 561 438 gzip; CSS 180 862 / 29 600; reviewed baseline alatt. |
| `npm audit --audit-level=moderate`      | I-34 LOKÁLIS PASS       | 0 ismert vulnerability. |
| `npm run security:secrets`              | I-34 LOKÁLIS PASS       | 1 542 tracked és nem ignorált untracked text file magas bizonyosságú current-tree scan + tracked mobil signing-key fájlnév tiltás; history scan még nyitott. |
| `npm run lint:ratchet`                  | I-34 LOKÁLIS PASS       | 1 148 error / 98 warning; a diagnosztikai fingerprint változatlan, új vagy elmozdított diagnosztika nincs. |
...
| `npm run edge:check` / ratchet / test    | I-34 LOKÁLIS PASS KORLÁTTAL | 31/31 entrypoint és config, 0 Deno diagnostic; 65/65 remote import exact, 0 unpinned; Deno 2.9.3. Az első teljes suite 85/86 egy időzítésérzékeny cleanup-timeout teszt miatt; izolált 11/11 és ismételt teljes 86/86 PASS. Immutable source gate `5d38e629bc798af9e4536f51b819e2b99cd5378f95250478e09d05c3c26c6bce`, 89 fájl / 859 724 canonical byte. |
| Edge strukturált log és release SBOM    | I-34 LOKÁLIS PASS       | Adversarial AST log-safety 9/9, közvetlen és globális console/stdout/stderr escape tiltással; Edge SBOM unit 7/7, a 31 deployálható entrypoint/config teljes leltárával. |
...
| Schema provenance gate                  | I-34 LOKÁLIS PASS KORLÁTTAL | Unit 7/7 és exact-debt gate PASS 136 migráción: 25/1/41/2 ismert unproven surface. Az I-27–I-34 candidate RPC-k és private táblák nincsenek hitelesen regenerált generated types-ban, ezért az exact local interface nem helyettesíti a linked-schema regenerálást. |
...
| Clean lokális migration replay          | NEM IGAZOLT             | A korábbi `plugin_webhook_events` source-hiány megszűnt. A friss DB-only Supabase replay a legelső migrációnál a hiányzó Storage-sémán állt meg; teljes Supabase-stack 136/136 replay még nem futott. |
| Linked `db lint --level warning`        | FAIL                    | 7 error, 6 warning; read-only ellenőrzés                                                                                                                           |
| Live `effectime.app` release attestation | FAIL                    | A Lovable/Cloudflare root elérhető, de `/.well-known/effectime-release.json` továbbra is 404. A live artifact ezért nem köthető az I-27–I-34 candidate SHA-k egyikéhez sem; production attestation NO-GO. |
...
Az I-34 következő kontrollált hulláma a hosztolt CI lezárása, majd csak history
reconcile után a restored-staging DB-first acceptance. A runtime feature-status/
dependency, downgrade-cleanup UI, grandfathered service-getter policy, bounded
lock-timeout, determinisztikus concurrency harness és a történeti install/
uninstall count race külön P2-k; ezek hiánya nem lazíthatja vissza a szerveroldali
entitlement/published-only határt.
...
61 shared / 75 local-only / 82 remote-only; a teljes 136-migrációs Supabase
...
A v3.51.12 I-34 lokálisan validált source candidate. Nem módosítja ezt a NO-GO
