...
Munkabranch: `codex/csv-import-client-boundary`
...
install/uninstall számlálórace-t friss statement-snapshotra épülő lock-protokollal
  ...
  Az I-44 v3.51.22 local candidate az aktív Import Wizard feltöltés utáni
  ...
  1 190 teszt,
  type/lint/build/bundle, web 7/7, mobile 390/390 + 2/2, Edge 109/109,
  security/provenance/release és mind a hét izolált PostgreSQL-contract PASS.
  Hosted candidate és production deploy még nincs.
A generált
...
| R-052 | CSV-import kliensintegritás és fail-visible adatlekérés | aktív Wizard step-2 zsákutca; malformált 200 hamis készállapot; tömörített sorindex; stale parse/result race; guidance-row limit/index drift; üres/duplikált header és mapping-collision; raw Functions/provider UI detail; ignored category/position lookup error | BIZONYÍTOTT local candidate | single-attempt validáló adapter; source-index remap; csak matched handler 4xx definitive; minden unknown-4xx/5xx/network/malformed outcome-unknown; bounded allowlisted code/request ID; last-selection-wins és confirmation invalidation; versioned guidance marker; fail-closed header/mapping; lokalizált és hozzáférhető result UI; minden export-read fail-closed | igen a candidate-ben; wire request változatlan; a bizonyított egy terminális hiba/sor response-contract explicit lett | focused 5 fájl / 89; full 96/1190; type/lint/build/bundle; web 7/7; mobile 390/390 + 2/2; Edge 109/109; security/provenance/release és hét DB-contract PASS | igen | `codex/csv-import-client-boundary`; production NO-GO | clean-head release, hosted candidate; majd paginált export és transactional server boundary |
...
| Nagy exportok lapozása és csonkolásdetektálása | Az I-44 fail-closed hibakezelése mellett a primary/category/position exportlekérdezések egyetlen, explicit `range`/count nélküli Supabase `select` hívást használnak. A szolgáltatói sorlimit hibamentes, de részleges `data` választ adhat; így egy nagy export észrevétlenül hiányos lehet. **BIZONYÍTOTT kliens-contract hiány; a tényleges live limit és érintett tenant-méret nincs bizonyítva.** | `data-fetcher`, Export Wizard, közös web/Android/iOS API | csendes adatkihagyás és hibás audit/export; M | stabil rendezés + bounded oldalankénti olvasás; exact count vagy limit+1 csonkoláskapu; oldalismétlés/hiány/race contract; nagy fixture; progress/cancel UX; audit csak teljes export után |
...
| I-44   | CSV import client integrity boundary | dependency-free response/error adapter; exact accounting és bounded row-error parse; original source-index remap; no-retry outcome-unknown; direct upload→mapping; last-selection-wins + confirmation invalidation; versioned guidance/source offset; duplicate header/mapping guards; safe localized+a11y result UI; export/data-fetcher fail-closed | focused 5 fájl / 89; full 96/1190; type/lint/build/bundle; web 7/7; mobile 390/390 + 2/2; Edge 109/109 és 31/31; security/provenance/release és hét DB-contract PASS | nincs DB/API/Edge/request/dependency változás; source/test/i18n commit revert; bizonytalan importkimenetnél automatikus retry tilos | local candidate `codex/csv-import-client-boundary`; hosted/production nincs |
...
| CSV import I-44 client integrity contract | LOKÁLIS PASS | Focused 5 fájl / 89 teszt; full coverage 96 fájl / 1190 teszt (56,50% statement/line, 80,84% branch, 39,56% function); typecheck és célzott ESLint; csökkentett lint-ratchet 1121/98; build; review-zott bundle 4 570 088 raw / 1 310 139 gzip, largest 1 783 491 / 567 359, CSS 180 900 / 29 610; web 7/7; mobile source 228, determinisztikus sync, foundation 390 és E2E 2/2; Edge 109/109, entrypoint 31/31, ratchet 24/24; hét DB-contract; 0 dependency vulnerability; current 1585 fájl és history 894 commit / 3754 blob / 219 179 108 byte secret scan; provenance/release PASS. Clean-head és hosted kapu pending. |
...
10. Az exact `csv_import` + `members_list` source-kapu az I-42 candidate-ben,
    ...
    I-43 source + hosted candidate-ben elkészült. Az I-44 local candidate az
...
A v3.51.22 I-44 local candidate az Import Wizard kliensintegritási határát
...
228/228 + 390/390 + 2/2 lokálisan PASS. A teljes request timeout szándékosan
...
következő P1 csomag. Clean-head és hosted candidate még nincs, production deploy
nem történt, ezért a kiadási döntés **NO-GO**.
