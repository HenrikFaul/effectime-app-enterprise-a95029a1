...
Munkabranch: `codex/payroll-export-integrity`
...
install/uninstall számlálórace-t friss statement-snapshotra épülő lock-protokollal
  ...
  actor és atomi snapshot/audit nélkül production **NO-GO**.
  Az I-47 v3.51.25 local source candidate az aktív Export Wizard és a legacy
  leave export minden ismert adatforrását közös exact-count lapozási határ mögé
  teszi. Az első exact count után stabil, 500 soros, nem átfedő oldalak készülnek;
  a pager a rövid/túl hosszú oldalt, malformált vagy hibás providerválaszt,
  blank/duplikált immutable ID-t, többoldalas count-driftet és a 100 000 soros
  felső korlát átlépését audit és letöltés előtt elutasítja. A táblák final countja
  HEAD; a linked sémában VOLATILE export RPC-ké bounded egy-soros POST probe.
  Legacy enrichment 200 useres batchben, globális identity/cap ellenőrzéssel fut;
  a profilprojekció továbbra is csak `user_id, display_name`. A request-ID nélküli
  leave RPC teljes exportprojekciós rendezési tuple-je statikus adatra
  megfigyelésileg determinisztikus, de nem helyettesít MVCC snapshotot. Minden
  projekció own-property + scalar/null runtime sémát kapott; a dátumok valódi ISO
  napok és az intervallum nem lehet fordított. Az egy-sheetes `.xls` fejlécével
  és opcionális guidance sorával együtt 65 536 sorra fail-closed, a legacy napi
  bontás pedig dátum- és expanded-output budgettel, lineáris active-interval
  sweeppel váltja a korábbi `napok × kérelmek` bejárást. A központi Excel XML
  generátor és a current-data template út is exact worksheet-limitet érvényesít;
  kompatibilis half-day null alakból nem készül hamis `afternoon` adat. Focused
  6 fájl / 175/175 és teljes single-worker 103 fájl / 1361/1361 coverage,
  typecheck, célzott ESLint, változatlan 1108/98 lint-ratchet, build/bundle,
  web 7/7, mobil 228/390/2, Edge 109/109 + 31/31, security/provenance/release és
  0 dependency vulnerability lokálisan PASS. A clean implementation commit
  release contractja 410/410; az exact `041b773…` source head hosted runja
  (`29970235413`) 12/12 PASS, 0 annotáció, potential merge `edce77a…`. A végső
  dokumentációs `ed5ec517…` head runja (`29970573620`) szintén 12/12 PASS; az
  első attempt külső `esm.sh` HTTP 522 hibája rerunra megszűnt.
  Az I-48 v3.51.26 local source candidate a külön time-attendance payroll
  exportot exact-count 500-row pagerrel, 100 000 source-row és 65 535 XLS
  data-row limittel, 23 üzleti + 2 UUID mezős canonical runtime projekcióval,
  timestamp-, nonnegative-, email-duplikáció- és dokumentált aritmetikai
  validációval védi. A tiszta artifact UUID-receipttel rögzül a delivery előtt;
  a delivery port aszinkron natív adaptert is awaitel. Entitlement, tenant-scope,
  list/export staleness, same-tick single-flight és kétirányú mutation/export
  operation gate készült, nyolc locale + a11y állapotokkal. Focused 4 fájl /
  113/113 és teljes izolált 106 fájl / 1458/1458 coverage, type/lint/build/bundle,
  web 7/7, mobile 228/390/2, Edge 109/109 + 31/31, security/provenance/release,
  0 dependency vulnerability és pinned PG18 payroll snapshot contract lokálisan
  PASS. A clean-worktree mobile release gate és hosted CI commit után kötelező.
  A kliensjavítás nem oldja meg az attendance payroll DDL-provenance, atomi exact
  period/revision snapshot, idempotency, `exported` correction, non-mutating CSV,
  batch-redownload és retention hiányát. A linked Supabase runtime HTTP 402
  kvótazárja, staging/history/Edge drift, hibás Vercel repo/domain kötés és 404
  release marker miatt production **NO-GO**.
A generált
...
| R-007 | Jelenlét és payroll export            | screenshot, TA/payroll docs       | BIZONYÍTOTT                         | attendance/payroll UI+Edge+RPC                  | igen                  | contract+unit+Edge+PG18 | igen      | két payroll modell: Edge snapshot hardened; attendance kliensintegritás I-48, szerver boundary nyitott | attendance DDL+atomi export+szemantikai döntés+staging |
...
| R-053 | Reports/Audit/Export exact hozzáférés és legacy leave-export kliensintegritás | sibling permission + umbrella entitlement mount-bypass; ignored provider/audit hibák; rejected transport/null read hamis siker; audit utáni letöltés helyett korábbi letöltés; formula-leading CSV; hibás dátum-containment; `.xlsx` címke `.xls` artifacthoz; XML 1.0-tiltott vezérlőkarakter; stale actor/workspace és dupla submit | BIZONYÍTOTT source + hosted candidate | capabilitynként permission × exact entitlement; külön komponenshatár; típusos fail-closed query/audit/artifact/download adapter; audit-before-download; közös CSV neutralizer; overlap query; XML 1.0 szűrés + escaping; igaz `.xls`; generation + symbol-token single-flight; nyolc locale stabil hiba | kliens runtime-ban igen; DB/API/Edge változatlan | focused 6 fájl / 83; full 99/1223; type/lint/build/bundle; web 7/7; mobile source 228 + sync + release 410/410 + E2E 2/2; hosted 12/12, 0 annotáció PASS | igen | draft PR #194, head `52a8f72…`, run `29961698732`; production NO-GO | I-47 klienslapozás kész; server-owned exact permission+entitlement export RPC/job, auth.uid actor, atomi audit+snapshot és restored staging nyitott |
| R-054 | Settings Export Wizard exact hozzáférés, audit- és delivery-integritás | `canAccessExport` nem jutott el a Settings útvonalra; download audit előtt; audit `{error}` ignored; null provider adat üres siker; mode-selection bypass; same-tick dupla submit és stale scope | BIZONYÍTOTT source + hosted candidate | independent `canExport`/`canImport`; mode/entity triple recheck; typed artifact→strict audit→download executor; exact `error: null`; truthful additive artifact/delivery metadata; mounted+actor/workspace/entity token; észlelt stale után nincs új audit, in-flight audit után nincs download/UI-success | kliens runtime-ban igen; DB/API/Edge változatlan | focused 8 fájl / 120; full 101/1267; type/lint/build/bundle; web 7/7; mobile 2/2 + release 410/410; Edge 109/109 + 31/31; security/provenance; hosted 12/12, 0 annotáció PASS | igen | draft PR #195, head `0a4994d…`, merge `a120686…`, run `29965167738`; production NO-GO | server-owned authorization/actor/snapshot/audit külön P1 |
| R-055 | Export Wizard + legacy leave bounded exact-count lapozással és csonkolásdetektálással | szolgáltatói implicit row limit; explicit range/count hiánya; nagy export csendes részhalmaz-kockázata; legacy enrichment és napi artifact-expanzió; XLS worksheet-limit; runtime provider-sor séma | BIZONYÍTOTT source + hosted candidate | közös 500-row exact-count pager; 100 000 source-row cap; expected page length; stabil order + immutable identity (a request-ID nélküli leave RPC-nél teljes projekciós tuple); globális duplicate guard az identity-s forrásokon; multi-page final count; table HEAD és VOLATILE RPC POST probe; 200-user legacy batch; privacy-allowlisted profile projection; own-property scalar/date parser; központi XLS 65 536 total-row budget; legacy bounded active-interval sweep | kliens runtime-ban igen; DB/API/Edge/dependency változatlan | focused 6/175; full 103/1361; type/lint/build/bundle; web 7/7; mobile 228/390/410/2; Edge 109/109 + 31/31; security/provenance/release; hosted 12/12, 0 annotáció PASS | igen | draft PR #196; implementation `041b773…`, merge `edce77a…`, run `29970235413`; production NO-GO | authenticated isolated 501+ PostgREST contract; mért query plan/latency; server-owned exact authorization/actor, egy snapshot/job és atomi audit receipt; külön payroll P1 |
| R-056 | Attendance payroll kliensintegritás, delivery és műveleti konkurencia | one-shot RPC cast; implicit row cap; download-before-record; raw hibák; entitlement- és stale-scope hiány; export↔mutation race; aszinkron natív delivery kezelhetetlensége | BIZONYÍTOTT local source candidate | exact-count 500-row pager + final count; 100k/65,535 cap; exact 25-field canonical projection; UUID/period/status/timestamp/nonnegative/email/arithmetic validation; typed artifact→UUID receipt→awaited delivery orchestrator; entitlement fail-closed; list/export tenant token; kétirányú operation gate; recovery metadata; nyolc locale+a11y | kliens runtime-ban igen; DB/API/Edge változatlan | focused 4/113; full isolated 106/1458; type/lint/build/bundle; web 7/7; mobile 228/390/2; Edge 109/109 + 31/31; secrets/provenance/release; PG18 payroll snapshot contract PASS | igen | v3.51.26 local candidate; clean release/hosted CI commit után; production NO-GO | attendance DDL provenance; egy exact period/revision szervertranzakció; idempotency; exported correction; non-mutating CSV; batch redownload; retention; real device delivery |
...
| Authoritatív export permission, entitlement, actor és snapshot | Az I-45/I-46 minden ismert UI-mountot exact permission × entitlement mögé zár és audit nélkül nem tölt le, az I-47 pedig teljességi lapozást ad, de a browser readet továbbra is az entitások RLS-e szabályozza, nem egy authoritative export boundary. Az audit INSERT policy workspace-tagságot ellenőriz, de a caller által adott `actor_id`/`action` mezőt nem köti `auth.uid()`-hoz. A többquerys olvasás nem egy MVCC snapshot. **BIZONYÍTOTT szerver-authorizációs és auditintegritási hiány.** | `legacyLeaveExport`, `entity-export`, data-fetcher, entity RLS, `enterprise_audit_events`, közös web/Android/iOS adatút | jogosultságeltérés, hamis actor, same-count mutáció vagy részleges/driftelt compliance export; L; DB history + restored staging függőség | szerver-owned, verziózott RPC/job: auth.uid-ból actor, exact permission+entitlement, egy snapshot/materializált job, bounded row ceiling, atomi audit receipt; két-tenant/RBAC/entitlement/demotion/concurrency/forged-actor PG-contract; DB→client rollout és same-SHA acceptance |
| Nagy exportok klienslapozása és szerver-snapshot acceptance | Az I-47 a szolgáltatói implicit row limitet közös 500 soros exact-count pagerrel, stabil rendezéssel/identityvel, oldalhossz-, runtime row-schema-, globális duplikáció- és final-count kapuval kezeli. Táblákhoz HEAD, VOLATILE RPC-khez egy-soros POST count probe, legacy enrichmenthez 200-user batch készült. A single-sheet XLS total 65 536 sorba számolja a fejlécet/guidance-ot; a legacy dátum- és overlap-expanzió bounded active-interval sweep. **BIZONYÍTOTT kliens-contract javítás.** Ugyanakkor több HTTP-kérés nem egy MVCC snapshot; same-count delete/insert vagy value update, a 200 offset-RPC várható query-plan/latency terhe és a valódi authenticated PostgREST viselkedés még nincs izolált 501+ fixture-rel igazolva. | `exportPagination`, `exportArtifactLimits`, `exportRowValidation`, `data-fetcher`, `entity-export`, `legacyLeaveExport`, közös web/Android/iOS API | statikus nagy adathalmaz csonkolása és túlméretes artifact auditálása detektált; konkurens same-count drift, terhelés és gateway-kontrakt marad; M | authenticated izolált 501/1001 soros PostgREST acceptance és query-plan/latency mérés; server-owned egy-snapshot RPC/job exact authorizációval, bounded materializálással és atomi audit receipt-tel |
| Attendance payroll szerverintegritás és üzleti állapotgép | Az I-48 lezárta a kliens one-shot castot, implicit capot, download-before-record sorrendet, entitlement/stale scope-ot és export↔mutation race-t. **BIZONYÍTOTT nyitott szerverhatár:** az attendance payroll table/RPC DDL migration provenance 0; fetch és record külön HTTP/RPC, a record minden aktuális locked periódust érint exact artifact period ID/revision nélkül; nincs operation UUID/idempotency; `p_only_locked=true` kompatibilisen exported sort is adhat, miközben exported→draft nincs; a teljes CSV ugyanazzal a mutáló record RPC-vel fut; a batch ID-hoz nincs read/redownload UI. | `time-attendance/api.ts`, `payroll-export.ts`, `AdminOverview.tsx`, generated RPC types, payroll docs, `db-audit` | audit-artifact/status eltérés concurrency alatt, duplikált batch, nem javítható exported periódus, elemző CSV rejtett mutációja, kliens által küldött audit payload; P0/P1; DDL history + termékdöntés + staging függőség | verziózott attendance payroll DDL explicit tenant/RBAC/entitlement/ACL/search_path contracttal; egy tranzakció/job exact period ID+revision+hash és szerver-materializált payload mellett; operation UUID unique/replay; reason-bound correction+prior batch; külön non-mutating analysis; tenant-safe batch history/redownload; GDPR retention; restored-staging concurrency és real device delivery acceptance |
...
| I-46   | Export Wizard exact capability és delivery boundary | Settings `canAccessExport` prop; independent export/import loss recovery; entity-mode recheck; typed artifact→audit→download executor; exact `error: null`; compatibility + truthful artifact metadata; scope-token/single-flight; észlelt stale után nincs új audit, in-flight audit után nincs download/UI-success; outcome-safe i18n és a11y | focused 8 fájl / 120; full 101/1267, coverage 57,21/81,24/40,99; typecheck; lint 1108/98; build + JS 4 578 450 / 1 313 642, hosted 4 578 543 / 1 313 727; web 7/7; mobile sync + 2/2 + clean release 410/410; Edge 109/109 + 31/31; security/provenance; hosted 12/12 PASS | nincs DB/API/Edge/dependency változás; commit revert; már megírt audit/letöltési kérés nem vonható vissza; server actor/snapshot/audit nyitott | draft PR #195, implementation `c7e2caa…`, head `0a4994d…`, merge `a120686…`, run `29965167738`; production NO-GO |
| I-47   | Export Wizard + legacy leave pagination és artifact-integrity boundary | közös 500-row exact-count pager; 100 000-es hard cap; exact page length; stable order + immutable identity/full leave tuple; global duplicate/final-count kapu; table HEAD / VOLATILE RPC one-row POST probe; 200-user enrichment; privacy projection; own-property scalar/null + ISO-date parser; központi XLS header/guidance-aware 65 536 total-row limit; legacy date/output cap, stable active-interval sweep és non-fabricating half-day mapping; exact-null audit | focused 6 fájl / 175/175; full 103/1361, coverage 57,52/82,12/43,30; typecheck; célzott ESLint; lint 1108/98 unchanged; local JS 4 587 149 / 1 316 233, hosted 4 587 242 / 1 316 272; web 7/7; mobile 228/390/410/2 + clean sync; Edge 109/109 + 31/31; security/provenance/release; hosted 12/12, 0 annotáció PASS | nincs DB/API/Edge/dependency változás; commit revert; multi-request same-count mutációt nem detektál; total byte/per-cell és timeout P2; server snapshot/job és atomi audit nyitott; payroll külön P1 | draft PR #196; implementation `041b773…`, merge `edce77a…`, run `29970235413`; production NO-GO |
| I-48   | Attendance payroll client-integrity boundary | exact 500-row pager/final count; 100k source + 65,535 XLS data-row cap; exact 23 business + 2 UUID canonical projection; state timestamp, nonnegative, normalized-email uniqueness és 0.05h arithmetic invariant; pure typed artifact→UUID receipt→awaited sync/async delivery; bounded post-record recovery; exact entitlement; tenant/list/export staleness; export↔mutation shared operation gate; eight-locale+a11y; global serial jsdom cleanup | focused 4 fájl / 113; full isolated 106/1458, coverage 58,05/82,59/45,03; typecheck; targeted ESLint; reduced lint 1100/98; build + JS 4 600 314 / 1 320 496, largest 1 789 794 / 569 555; web 7/7; mobile 228/390/2 + deterministic sync; Edge 109/109 + 31/31; 0 vulnerability; current/history secrets; provenance/release; PG18 payroll snapshot PASS | nincs DB/API/Edge/dependency változás; source commit revert; recorded batch nem vonható vissza; DOM save nem attestálható; clean release gate commit után; attendance DDL/atomicity/idempotency/state-machine/CSV/recovery/retention P0/P1 nyitott | local candidate `codex/payroll-export-integrity`; commit/push/hosted CI következik; Supabase 402 + staging/drift + provider/SHA blockers miatt production NO-GO |
...
| Export pagination I-47 contract | LOKÁLIS + HOSTED PASS | Focused 6/175; full 103/1361; type/lint/build/bundle; web 7/7; mobile 228/390/410/2; Edge 109/109 + 31/31; 0 vulnerability; secrets/provenance/release PASS. Implementation run `29970235413` és final docs-head `29970573620` egyaránt 12/12 PASS; az első final attempt külső `esm.sh` 522-je rerunra megszűnt. |
| Attendance payroll I-48 client contract | LOKÁLIS PASS, HOSTED FÜGGŐBEN | Focused 4/113; full isolated 106/1458 (58,05% statement/line, 82,59% branch, 45,03% function); typecheck; targeted ESLint; lint-ratchet 1100/98; build; reviewed JS 4 600 314 / 1 320 496, largest 1 789 794 / 569 555; web 7/7; mobile source 228, sync drift 0, foundation 390 és E2E 2/2; Edge 109/109, 31/31, diagnostic 0; 0 vulnerability; current 1603 file és history 905 commit / 3872 blob / 234 869 264 byte secret scan; migration/schema/release PASS; pinned PG18 payroll snapshot PASS. Clean release gate és hosted CI csak commit után igazolható. Attendance server DDL/atomicity/idempotency/state-machine/CSV/recovery/retention és real-device save nyitott. |
...
| Live `effectime.app` release attestation | FAIL                    | A Cloudflare root HTTP 200 és `x-deployment-id: 205b23c0-72e8-4158-853e-348a2358fd10`, de `/.well-known/effectime-release.json` továbbra is 404. A live artifact exact Git SHA-ja nem bizonyítható; production attestation NO-GO. |
| Production provider és Supabase runtime | P0 FAIL | A Vercel project a másik `HenrikFaul/effectime-app-enterprise` repositoryhoz/mainhez kötött, 0 envvel és `effectime.app` domain nélkül; a live domain nem oldható fel Vercel deploymentként. A linked Supabase management státusz `ACTIVE_HEALTHY`, de a tényleges public health és side-effect-free import endpoint HTTP 402 `exceed_db_size_quota`; branch/staging nincs. GitHub secret/variable/environment/deployment/release leltár üres. Deploy nem történt. |
...
**Azonnali P0 release-előfeltétel:** a linked Supabase kvóta helyreállítása és
valós Edge health 2xx bizonyítása; külön Effectime staging branch/projekt; majd
migration/DB-lint/Edge `verify_jwt` drift egyeztetés. Ezután köthető a helyes
repository és staging-scoped env a web providerhez, és csak attestált azonos
SHA-ból indulhat DB → Edge → web acceptance/promote. A jelenlegi Vercel projekt
más repositoryt és domaint céloz, ezért nem használható production deployra.
...
12. Az attendance payroll hivatalos exportja kizárólag az exact `locked`
    periódusokat fogadja el, vagy legyen auditált explicit re-export az
    `exported` állapotból? Re-export esetén kötelező-e indok, előző batch ID és
    ugyanazon operation UUID idempotens replay?
13. A „Teljes CSV” hivatalos, státuszt módosító payroll batch legyen, vagy külön
    non-mutating elemző export? A jelenlegi közös record RPC rejtetten
    `exported` állapotba viszi a havi locked periódusokat, ezért az „ad-hoc
    elemzés” ígéret döntés és külön szervercontract nélkül nem tartható.
...
A v3.51.24 I-46 source + hosted candidate a Settings alatti Export Wizardre is
átviszi az exact export/import capability-kat, mode/entity bypass nélkül. Az
artifact → exact-null audit → browser download sorrend, scope-token és same-tick
single-flight lezárja a második aktív exportút delivery-integritását. Focused
120/120, teljes 101/1267, type/lint/build/bundle, web 7/7, mobile release 410/410
+ 2/2, Edge 109/109 + 31/31, security/provenance lokálisan PASS. A végső exact
`17dacee…` head hosted runja (`29965519542`) 12/12 PASS, 0 annotáció; draft PR
#195. Ez source/CI candidate, production deploy nem történt.

A v3.51.25 I-47 source + hosted candidate az Import/Export Center Export Wizard és
a legacy leave exporter összes ismert forrását közös exact-count, stable-order,
runtime-row-schema és artifact-cap határ mögé teszi. A központi SpreadsheetML
limit a direct template/generator hívásokat is védi, a legacy interval sweep
exact output cap mellett őrzi a `(start_date,id)` sorrendet, és kompatibilis
half-day null alakból nem fabrikál időszakot. Focused 6 fájl / 175/175, teljes
103/1361, typecheck, célzott ESLint, változatlan lint-ratchet, build/bundle, web
7/7, mobile 228/390/410/2 clean sync mellett, Edge 109/109 + 31/31, dependency/
secret/provenance/release kapuk PASS. Az exact `041b773…` implementation head
hosted runja (`29970235413`) 12/12 PASS, 0 annotáció; retained release/
diagnostics/unsigned Android artifactok `8549433507` / `8549431377` /
`8549399088`. A final docs-head `ed5ec517…` runja (`29970573620`) szintén
12/12 PASS.

A v3.51.26 I-48 local candidate lezárja a külön attendance payroll one-shot
castot, implicit pager/cap hiányt, download-before-record sorrendet, entitlement-
és tenant-stale hibát, valamint mindkét export↔mutation race irányt. Exact 25
mezős canonical payload, UUID/timestamp/nonnegative/normalized-email és
aritmetikai kapu, tiszta artifact→UUID receipt→awaited web/native delivery,
bounded recovery metadata és nyolc locale+a11y készült. Focused 113/113, teljes
106/1458, type/lint/build/bundle, web 7/7, mobile 228/390/2, Edge 109/109 +
31/31, dependency/secret/provenance/release és pinned PG18 payroll snapshot
lokálisan PASS; clean release és hosted CI commit után következik. Ez nem
helyettesíti a verziózott attendance payroll DDL-t, egy exact period/revision
szervertranzakciót, idempotenciát, exported-correction és non-mutating CSV
döntést, batch redownloadot/retentiont vagy valós iOS/Android save acceptance-et.
A linked Supabase HTTP 402 kvótazár, staging/migration/Edge drift, rossz Vercel
repo/domain kötés és 404 release marker miatt production továbbra **NO-GO**.
