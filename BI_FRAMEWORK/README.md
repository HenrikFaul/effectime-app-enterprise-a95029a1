...
### Step 1 — Run the automated setup analyzer (30 seconds)
```bash
python BI_FRAMEWORK/setup_analyzer.py
```
Scans CHANGELOG.md, versioning/, supabase/migrations/, src/app/, docs/, growth_strategy/, and valuation/ — then auto-fills `SYSTEM.md` Section 1 with your project's actual values. Outputs `setup_report.md` with detected metrics, database tables, confidence levels, and what needs manual review.

### Step 2 — Complete anything not auto-detected
Read `SETUP.md` for the full 9-step configuration guide. Most fields are auto-filled; you verify and correct.

### Step 3 — Read SYSTEM.md first in every AI session
...
### Step 4 — Route to the specialist prompt
Use the routing table in `SYSTEM.md` or this README to find the right prompt file for your task.

### Step 5 — Provide your context
...
BI_FRAMEWORK/
├── README.md                           This file
├── SETUP.md                            9-step configuration guide (run the analyzer first)
├── SYSTEM.md                           Master controller — give this to AI first, every session
├── setup_analyzer.py                   Auto-fills SYSTEM.md by scanning the repository
├── setup_report.md                     Generated by setup_analyzer.py — findings + review checklist
├── prompts/
