...
import {
  ...
  waitForPostgres,
} from "./profiles-tenant-db-contract.mjs";

const containerName = "effectime-profiles-tenant-pg18-42-001122aabbcc";
const containerId = "a".repeat(64);
const foreignContainerId = "b".repeat(64);
const ownershipToken = "c".repeat(32);

test("creates a collision-resistant, validated profiles tenant identity", () => {
  ...
  assert.throws(
    ...
    /Invalid profiles tenant contract container identity/,
  );
  assert.throws(
...
test("rejects injected names and accepts only exact Docker creation IDs", () => {
  assert.throws(
    ...
    /Invalid profiles tenant contract container name/,
  );
  assert.equal(parseCreatedContainerId(`${containerId}\n`), containerId);
  assert.throws(
    () => parseCreatedContainerId(containerName),
    /Invalid profiles tenant contract container ID/,
  );
  assert.throws(
