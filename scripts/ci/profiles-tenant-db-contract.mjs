...
export const CONTRACT_DATABASE = "effectime_profiles_tenant_contract";
export const CONTRACT_SQL_PATH = "/contract/fixture.sql";
export const MIGRATION_SQL_PATH = "/contract/migration.sql";
export const OWNERSHIP_LABEL_KEY = "com.effectime.ci.profiles-tenant-contract";

const repositoryRoot = resolve(dirname(fileURLToPath(import.meta.url)), "../..");
const containerNamePattern = /^effectime-profiles-tenant-pg18-[1-9][0-9]*-[0-9a-f]{12}$/;
...
function assertContainerName(containerName) {
  if (!containerNamePattern.test(containerName)) {
    throw new Error("Invalid profiles tenant contract container name");
  }
...
function assertContainerId(containerId) {
  if (!containerIdPattern.test(containerId)) {
    throw new Error("Invalid profiles tenant contract container ID");
  }
  return containerId;
}
...
function assertOwnershipToken(ownershipToken) {
  if (!ownershipTokenPattern.test(ownershipToken)) {
    throw new Error("Invalid profiles tenant contract ownership token");
  }
...
export function createContainerName({
  ...
  if (!Number.isInteger(pid) || pid < 1 || !/^[0-9a-f]{12}$/.test(suffix)) {
    throw new Error("Invalid profiles tenant contract container identity");
  }
  return `effectime-profiles-tenant-pg18-${pid}-${suffix}`;
}
...
export function buildDockerRunArgs({ containerName, repoRoot, password, ownershipToken }) {
  ...
  if (!repoRoot || !password) {
    throw new Error("Profiles tenant contract runtime inputs are required");
  }

  const fixtureSource = resolve(repoRoot, "scripts/ci/profiles-tenant-migration.test.sql");
...
export function buildPsqlCommandArgs(containerName, command, { tuplesOnly = false } = {}) {
  assertContainerName(containerName);
  if (!command) {
    throw new Error("A profiles tenant contract command is required");
  }
  return [
    "exec",
    containerName,
...
export function resolveOwnedCleanupTarget({ containerId, ownershipToken, inspectionOutput }) {
  ...
  if (unexpected.length > 0 || inspectedId !== containerId || inspectedToken !== ownershipToken) {
    throw new Error(
      "Refusing to remove a container whose ID and profiles tenant ownership label do not match",
    );
  }
  return containerId;
}
...
export function cleanupOwnedContainer(
  ...
  if (inspection.status !== 0) {
    throw new Error(
      `Unable to prove profiles tenant contract container ownership: ${inspection.stderr?.trim() || "inspect failed"}`,
    );
  }
  ...
  if (cleanup.status !== 0) {
    throw new Error(
      `Failed to remove owned profiles tenant contract container ${containerId}: ${cleanup.stderr?.trim() || "unknown error"}`,
    );
  }
  return true;
}
...
export async function waitForPostgres(
  ...
  if (!Number.isInteger(attempts) || attempts < 1 || intervalMilliseconds < 0) {
    throw new Error("Invalid profiles tenant readiness limits");
  }
  ...
  throw new Error(
    `PostgreSQL profiles tenant contract database ${CONTRACT_DATABASE} did not become queryable`,
  );
}

export async function runProfilesTenantDatabaseContract({
  ...
  console.log(`Profiles tenant DB contract container: ${containerName}`);
  ...
  try {
    ...
    console.log("Profiles tenant PostgreSQL 18.4 contract passed.");
  } catch (error) {
    contractError = error;
  } finally {
    try {
      if (
        ...
        console.log(
          `Removed owned profiles tenant contract container: ${containerName} (${createdContainerId})`,
        );
      }
    } catch (error) {
      cleanupError = error;
  ...
  if (contractError && cleanupError) {
    throw new AggregateError(
      ...
      "Profiles tenant database contract and owned-container cleanup both failed",
    );
  }
...
if (invokedUrl === import.meta.url) {
  runProfilesTenantDatabaseContract().catch((error) => {
