...
export const CONTRACT_DATABASE = "effectime_payroll_contract";
export const CONTRACT_SQL_PATH = "/contract/fixture.sql";
export const MIGRATION_SQL_PATH = "/contract/migration.sql";
export const OWNERSHIP_LABEL_KEY = "com.effectime.ci.payroll-contract";

const repositoryRoot = resolve(dirname(fileURLToPath(import.meta.url)), "../..");
const containerNamePattern = /^effectime-payroll-pg18-[1-9][0-9]*-[0-9a-f]{12}$/;
...
export const CONCURRENCY_SCENARIOS = Object.freeze([
  Object.freeze({
    ...
    sessionAVariable: "PAYROLL_CONCURRENCY_A=1",
    sessionBVariable: "PAYROLL_CONCURRENCY_B=1",
    verifyVariable: "PAYROLL_CONCURRENCY_VERIFY=1",
    advisoryObjectId: 1,
    waitingApplication: "effectime-payroll-contract-b",
    waitDescription: "period row lock",
    applications: Object.freeze(["effectime-payroll-contract-a", "effectime-payroll-contract-b"]),
  }),
  Object.freeze({
    label: "payroll break-glass reopen",
    sessionAVariable: "PAYROLL_REOPEN_CONCURRENCY_A=1",
    sessionBVariable: "PAYROLL_REOPEN_CONCURRENCY_B=1",
    verifyVariable: "PAYROLL_REOPEN_CONCURRENCY_VERIFY=1",
    advisoryObjectId: 2,
    waitingApplication: "effectime-payroll-reopen-contract-b",
    waitDescription: "period row lock",
    applications: Object.freeze([
      "effectime-payroll-reopen-contract-a",
      "effectime-payroll-reopen-contract-b",
    ]),
  }),
  Object.freeze({
    label: "payroll actor demotion versus reopen",
    sessionAVariable: "PAYROLL_DEMOTION_CONCURRENCY_A=1",
    sessionBVariable: "PAYROLL_DEMOTION_REOPEN_CONCURRENCY_B=1",
    verifyVariable: "PAYROLL_DEMOTION_CONCURRENCY_VERIFY=1",
    advisoryObjectId: 3,
    waitingApplication: "effectime-payroll-demotion-reopen-contract-b",
    waitDescription: "membership row lock",
    applications: Object.freeze([
      "effectime-payroll-demotion-contract-a",
      "effectime-payroll-demotion-reopen-contract-b",
    ]),
  }),
]);
const concurrencyApplications = CONCURRENCY_SCENARIOS.flatMap((scenario) => scenario.applications);

function buildAdvisoryLockQuery(objectId) {
  if (!Number.isInteger(objectId) || objectId < 1) {
    throw new Error("Invalid payroll concurrency advisory lock identity");
  }
  return [
    "SELECT EXISTS (",
...
function buildConcurrentWaitQuery(applicationName, blockingApplicationName) {
  if (!/^[a-z0-9-]+$/.test(applicationName) || !/^[a-z0-9-]+$/.test(blockingApplicationName)) {
    throw new Error("Invalid payroll concurrency application name");
  }
  return [
    "SELECT EXISTS (",
    "SELECT 1 FROM pg_stat_activity AS waiter",
    "WHERE waiter.datname = current_database()",
    `AND waiter.application_name = '${applicationName}'`,
    ...
    `WHERE blocker.application_name = '${blockingApplicationName}'`,
    ")",
    ");",
  ].join(" ");
}
const resetConcurrencyGateQuery =
  "UPDATE public.fixture_payroll_concurrency_gate SET released = false WHERE id = 1;";
const releaseConcurrencyGateQuery =
  "UPDATE public.fixture_payroll_concurrency_gate SET released = true WHERE id = 1;";
...
function assertContainerName(containerName) {
  if (!containerNamePattern.test(containerName)) {
    throw new Error("Invalid payroll contract container name");
  }
...
function assertContainerId(containerId) {
  if (!containerIdPattern.test(containerId)) {
    throw new Error("Invalid payroll contract container ID");
  }
  return containerId;
}
...
function assertOwnershipToken(ownershipToken) {
  if (!ownershipTokenPattern.test(ownershipToken)) {
    throw new Error("Invalid payroll contract ownership token");
  }
...
export function createContainerName({
  ...
  if (!Number.isInteger(pid) || pid < 1 || !/^[0-9a-f]{12}$/.test(suffix)) {
    throw new Error("Invalid payroll contract container identity");
  }
  return `effectime-payroll-pg18-${pid}-${suffix}`;
}

export function createOwnershipToken(bytes = randomBytes(16)) {
  const ownershipToken = Buffer.from(bytes).toString("hex");
  return assertOwnershipToken(ownershipToken);
}
...
export function buildDockerRunArgs({ containerName, repoRoot, password, ownershipToken }) {
  ...
  if (!repoRoot || !password) {
    throw new Error("Container name, repository root and ephemeral password are required");
  }
  const contractSource = resolve(repoRoot, "scripts/ci/payroll-snapshot-migration.test.sql");
...
export function buildPsqlCommandArgs(containerName, command, { tuplesOnly = false } = {}) {
  assertContainerName(containerName);
  if (!command) throw new Error("A PostgreSQL contract command is required");
  return [
    "exec",
    containerName,
    "psql",
...
export function resolveOwnedCleanupTarget({ containerId, ownershipToken, inspectionOutput }) {
  ...
  if (unexpected.length > 0 || inspectedId !== containerId || inspectedToken !== ownershipToken) {
    throw new Error("Refusing to remove a container whose ID and ownership label do not match");
  }
  return containerId;
}
...
export function cleanupOwnedContainer(
  ...
  if (inspection.status !== 0) {
    throw new Error(
      `Unable to prove payroll contract container ownership: ${inspection.stderr?.trim() || "inspect failed"}`,
    );
  }
  ...
  if (cleanup.status !== 0) {
    throw new Error(
      `Failed to remove owned payroll contract container ${containerId}: ${cleanup.stderr?.trim() || "unknown error"}`,
    );
  }
  return true;
}
...
export function observeDockerChild(child, args) {
  ...
  // Attach both fulfillment and rejection handlers synchronously. Consumers get
  // an always-fulfilled outcome promise, so a child cannot become an unhandled
  // rejection while the runner is waiting for a database concurrency barrier.
  ...
  for (let attempt = 1; attempt <= 150; attempt += 1) {
    ...
    const result = dockerSync(
      buildPsqlCommandArgs(
        containerName,
        buildConcurrentWaitQuery(scenario.waitingApplication, scenario.applications[0]),
        { tuplesOnly: true },
      ),
      { allowFailure: true },
    );
...
export async function settleOrTerminateSessions(
  ...
  if (pending.length === 0) return Promise.all(sessions.map((session) => session.completion));
  ...
  if (pending.length > 0) {
    try {
      ...
      if (termination.status !== 0) {
        terminationError = new Error(
          `Failed to terminate payroll concurrency backends: ${termination.stderr?.trim() || "unknown error"}`,
        );
      }
    } catch (error) {
    ...
    await Promise.all(
      pending.map((session) => waitForObservedCompletion(session, terminationGraceMilliseconds)),
    );
    pending = sessions.filter((session) => !session.settled);
...
async function runConcurrencyScenario(containerName, scenario) {
  ...
  try {
    ...
    try {
      const release = dockerSync(buildPsqlCommandArgs(containerName, releaseConcurrencyGateQuery), {
        allowFailure: true,
      });
      if (release.status !== 0) {
...
export async function runPayrollSnapshotDatabaseContract({
  ...
  console.log(`Payroll snapshot DB contract container: ${containerName}`);
  ...
  try {
    ...
    console.log(`Created owned payroll contract container ID: ${createdContainerId}`);
    await waitForPostgres(containerName);
    dockerSync(buildPsqlArgs(containerName), { stdio: "inherit" });
    await runConcurrencyContracts(containerName);
    console.log("Payroll snapshot PostgreSQL 18.4 contract passed.");
  } catch (error) {
    contractError = error;
  } finally {
    try {
      if (cleanupOwnedContainer({ containerId: createdContainerId, ownershipToken })) {
        console.log(
          `Removed owned payroll contract container: ${containerName} (${createdContainerId})`,
        );
      }
    } catch (error) {
      cleanupError = error;
  ...
  if (contractError && cleanupError) {
    throw new AggregateError(
      ...
      "Payroll database contract and owned-container cleanup both failed",
    );
  }
...
if (invokedUrl === import.meta.url) {
  runPayrollSnapshotDatabaseContract().catch((error) => {
