...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: doc, error: docErr } = await admin
      ...
      .select("id, workspace_id, content_html, doc_type")
      .eq("id", document_id)
      .single();
    ...
    const { data: membership } = await admin
      ...
      .select("id")
      .eq("workspace_id", (doc as any).workspace_id)
      .eq("user_id", user.id)
      .eq("status", "active")
      .maybeSingle();
    ...
    const originalHtml = (doc as any).content_html as string;
    const docType = (doc as any).doc_type as string;
    ...
    if (!aiResp.ok) {
      ...
      return jsonRes({ ok: false, ai_available: true, polished_html: originalHtml, error: "AI request failed" });
    }
    ...
    const { error: updateErr } = await admin
      ...
      .eq("id", document_id);
    if (updateErr) {
      console.warn("[document-ai-polish] DB update failed:", updateErr.message);
    }
