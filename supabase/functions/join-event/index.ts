    ...
    if (action === 'create-enterprise-instant-user') {
      ...
      if (!authorization.allowed) {
        ...
        return authorization.status === 503
          ...
          : jsonRes({ error: 'Nincs jogosultság instant user létrehozásához.' }, 403);
      }
      ...
      if (!registration) {
        console.error('Instant user provisioning registration failed.');
      ...
      if (createUserError || !createdUser.user) {
        console.error('Instant user auth creation failed.');
        const cleanup = await cleanupCreatedIdentity(createdUser.user?.id ?? null, null);
        if (!cleanup.ok) {
          console.error(`Instant user provisioning reconciliation pending: ${cleanup.reason}.`);
        }
        return jsonRes({ error: 'Nem sikerült létrehozni az auth usert.' }, 500);
      }
      ...
      if (profileError) {
        console.error('Instant user profile upsert failed.');
        const cleanup = await cleanupCreatedIdentity(instantUserId, null);
        if (!cleanup.ok) {
          console.error(`Instant user compensation incomplete: ${cleanup.reason}.`);
        }
      ...
      if (membershipError || !membership) {
        console.error('Instant user membership insertion failed.');
        const cleanup = await cleanupCreatedIdentity(instantUserId, null);
        if (!cleanup.ok) {
          console.error(`Instant user compensation incomplete: ${cleanup.reason}.`);
        }
      ...
      if (selectedRole) {
        ...
        if (allocationError) {
          console.error('Instant user role allocation insertion failed.');
          const cleanup = await cleanupCreatedIdentity(instantUserId, membership.id);
          if (!cleanup.ok) {
            console.error(`Instant user compensation incomplete: ${cleanup.reason}.`);
          }
      ...
      if (!provisioningCompleted) {
        console.error('Instant user provisioning completion was not proven.');
